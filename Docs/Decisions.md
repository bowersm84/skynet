...
### D-TCO-AUTOCLOSE — Work orders auto-close after 30 days pending TCO (2026-09-02)
**What:** tco_stale_candidates(p_days) computes per-WO eligibility: every non-cancelled job at pending_tco, ready_since older than p_days, no open shortfall, no open paperwork issue — with a human-readable reason for every skip. ready_since is the earlier of the job's SKY65 fulfillment stamp and the latest of its last compliance approval / actual_end (v2 clock fix: jobs the stranded-batch sweep advanced late must not restart the 30-day clock). auto_close_stale_tco(p_days, p_dry_run) performs the Approve TCO button's writes for each eligible WO — jobs pending_tco → complete, work order → complete, assemblies → complete — but records closed_by NULL and appends an explanatory tco_notes line so an auto-close is never mistakable for a compliance sign-off; one audit row 'tco_auto_closed' per WO and one bell to compliance/admin per run. Scheduled nightly at 06:00 UTC via pg_cron ('auto-close-stale-tco') on TEST and PROD. CO fulfillment is untouched — it posted on entry to pending_tco (SKY65). First PROD sweep (Sep 2) closed 66 work orders, skipped 49 with reasons, and the TCO Review list went 115 → 49 — a rolling 30-day window from here.
**Why:** 75 work orders sat in Pending TCO on PROD, the oldest from May, and the list had stopped meaning anything. Thirty days is long enough for compliance to act on anything that needs a human; past that, holding the WO open adds no control — the quantities and fulfillment posted at pending_tco entry — and only buries the reviews that matter. The two holds (shortfall, paperwork issue) keep every case a human flagged in human hands, and the NULL closed_by keeps the audit trail honest about what was signed and what was aged out.
**Files:** Docs/migrations/2026-09-02_auto_close_stale_tco.sql (functions + schedule), Docs/migrations/2026-09-02_auto_close_stale_tco_v2.sql (ready_since clock fix).

### D-SCHED — Early-started jobs and the week fetch: gap known, fix drafted and held (2026-08-28)
**What:** The Command grid's week fetch has no OR-arm for an ongoing job whose scheduled slot lies entirely after the visible window, so a job the floor starts ahead of its slot vanishes from the week view while physically occupying its machine. J-000207 (QL8C20-0FH, Mazak 1) ran in_setup on Aug 28 against a Sep 7–11 slot: invisible in Week, drawn correctly in 4-Week (arm 1 caught it and the D-SCHED-06/07 projection anchored the bar at its live setup_start). The instance was corrected by SQL — slot pulled to the job's own setup_start, span preserved (guarded one-shot, PROD). The class fix — replace the two backward-looking ongoing carryover arms with one unconditional 'fetch every ongoing job' arm — is drafted and anchor-verified as 2026-08-28_D-SCHED-early-start-fetch_CC_Prompt.md and deliberately HELD by Matt; run it the next time an early start makes a running job disappear.
**Why:** Data correction was enough for the day, and the fetch change touches the grid's hottest query — held rather than shipped under time pressure. Recording it here so the drafted fix isn't rediscovered from scratch.
**Files:** (held) 2026-08-28_D-SCHED-early-start-fetch_CC_Prompt.md; slot correction 2026-08-28_J-000207_slot_correction.sql.

### D-BLANK-INV-UI-02 — Blank inventory low-stock shading by blank type (2026-09-02)
**What:** Armory > Inventory > Blanks shades every lot row by its blank type's netted total (Σ available_bars over all lots sharing blank_type_id, falling back to material_type|bar_size for pre-catalog rows): amber below 5,000 pieces (BLANK_LOW_THRESHOLD), red below 3,000 (BLANK_CRITICAL_THRESHOLD). The Available cell takes the matching colour and, on multi-lot types, carries a "Σ n · k lots" chip so the basis of the shading is visible on the row. The summary strip gains Low / Critical type counts (over the rows shown) and a legend. loadInventory's category side-lookup now also returns blank_type_id. Type totals are computed over ALL blank rows, so the Type/Dash/Vendor/lot filters never change a row's shading. The existing isOut / isNeg treatments are unchanged (0 and negative both fall inside "critical"). Frontend-only, no migration. Companion read-only report for purchasing: 2026-09-02_blank_low_stock_report.sql (Block A one row per blank type with used_90d and last_received; Block B one row per lot under 5,000 carrying its type total).
**Why:** Matt wants a glance-level purchase signal on the blank shelf. The purchase decision is per blank type (series + material + dash/length), not per lot: judging the lot row would flag a 3,000-piece lot whose sibling lot holds 8,000, and would show a consumed kiosk stub (0 received, N used) as a spurious negative beside its later formal receipt — netting the type folds both into one correct number, and matches how forecast_blank_onhand already aggregates on-hand by series/material/length.
**Files:** src/pages/Armory.jsx.

### D-SCHED-22 — Week fetch: every ongoing job fetched unconditionally (2026-09-03)
**What:** The Command grid fetch's two backward-looking ongoing carryover arms (the null-end carryover and the D-SCHED-02 overrun arm) are replaced by one unconditional `status.in.(ONGOING_STATUSES)` arm, so every in_setup / in_progress / pending_passivation / in_passivation job is returned whatever the window. RESOLVES the D-SCHED gap entry held 2026-08-28: an ongoing job whose slot lies entirely AFTER the visible window was never fetched, so a job the floor started early vanished from the Week view while physically occupying its machine (J-000207 Aug 28; J-000158 / Nexturn 3 Sep 3 — the second occurrence, which triggered the held fix per its own run-when clause). No render change needed: the D-SCHED-06/07 projection anchors the fetched bar at its live setup_start, and on historic windows getLiveSpan is null so an out-of-window raw slot simply doesn't draw — past weeks unchanged. Cost: at most ~1 extra row per machine on the grid's hottest query. The Nexturn 3 instance was corrected by data one-shot the same day (2026-09-03_NT3_slot_correction.sql: J-000116 scheduled_end → actual_end first, then J-000158 slot → its recorded setup_start with span preserved — ordered so jobs_no_machine_overlap can never fire; proven against a scratch DB with the exclusion constraint active). Supersedes the held 2026-08-28_D-SCHED-early-start-fetch_CC_Prompt.md, which is discarded — its anchors predate the Aug-31 Command amendments.
**Why:** A running job must be on the grid in every view, wherever its slot sits; "fetch every ongoing job" is the whole rule, with no window arithmetic to get wrong. The plan data (scheduled_*) stays untouched by display — corrections to stale slots remain deliberate one-shots, not automatic mutations.
**Files:** src/pages/Schedule.jsx.

**Numbering:** issued as D-SCHED-22; the round brief said D-SCHED-09, which this log had already assigned to Completions never rewrite the schedule (2026-08-15). D-SCHED-21 was the highest in use, so this round takes 22, and the code comment in src/pages/Schedule.jsx cites D-SCHED-22 to match. Seventh occurrence of this collision across the log (see D-SCHED-16 through -21 and D-INV-03/-04); the convention question raised there — briefs checking the log before assigning a number, or code comments dropping the D-### citation — is still open.

### D-SCHED-23 — Slot hygiene: stale ends snapped, early starts pulled (2026-09-03)
**What:** One-shot PROD data rounds, every write audited under a new `schedule_slot_corrected` audit event: (1) Nexturn 3 pair correction — J-000116 `scheduled_end` → `actual_end` first, then J-000158's slot → its recorded `setup_start` with span preserved (predecessor-first ordering so the overlap rule cannot fire). (2) Shop-wide sweep from the same day's overlap diagnostic (71 pairs): 48 stale `scheduled_end` values on done jobs snapped to `actual_end` (rule-derived: earlier member of a live overlap pair, `actual_end` recorded, strictly inside the slot; deduplicated; includes RQ-36623267's 43-day phantom on Nexturn 1), 2 blocker snaps, and 3 early-started ongoing jobs pulled to their live starts with spans preserved (J-000153/Nexturn 6, J-000251/Bolt Master 5, J-000256/Bolt Master 1 — the 4th, 2nd, and 3rd instances of the D-SCHED-22 early-start class inside three days). 53 audit rows; every UPDATE ran in its own exception scope with per-row skip/blocked reporting; dry-run-first with a fingerprint match against the preview. 15 overlap pairs remain by design: all are inert under jobs_no_machine_overlap's occupying-status allowlist (D-SCHED-09) — owned by the 11 skip-listed jobs whose `actual_end` predates or overruns their own slot (start-side or wholesale slot errors; hand review, low value on terminal rows), the genuinely interleaved Nexturn 5 run (J-000083), and one start-side containment (Mazak 2) that no end-snap can reach.
**Why:** An early-started running job is an allowlist member, so its future slot phantom-reserves a machine window until completion, and before D-SCHED-22 it vanished from Week view entirely; stale terminal ends were pure data noise drowning diagnostics. No automation was added and none should be: D-SCHED-09's principle stands — completions never rewrite the schedule, the projection displays compression, and slot corrections remain deliberate, guarded, audited one-shots. (A kiosk-Complete auto-snap was considered and rejected against D-SCHED-09's plan-vs-actual record rationale; the removed trg_repack_on_completion is the cautionary precedent.)
**Deploy:** PROD (Supabase SQL Editor, guarded dry-run-first blocks; no app code).
**Files:** Docs/migrations/2026-09-03_NT3_slot_correction.sql, Docs/migrations/2026-09-03_slot_overlap_diagnostic.sql, Docs/migrations/2026-09-03_slot_hygiene_sweep.sql.

### D-SCHED-24 — Ignore action for run-target notices (2026-09-04)
**What:** The Command Messages panel's "Run target changed" rows gain an Ignore button beside Adjust End Date. Ignore stamps `jobs.run_target_ack_qty/_at/_by` (new additive columns, migration 2026-09-04_D-SCHED-24_ack_columns.sql) with the current run target and writes a `run_target_change_acknowledged` audit row — the end date is kept. `isScheduleStale` (src/lib/jobMerge.js) now treats a job as NOT stale when the current target equals `run_target_ack_qty`, so the panel row, the grid bar's amber triangle, and the Adjust End Date modal's warning all clear together. The ack is qty-anchored: any FURTHER target move (merge, unmerge, split) mismatches both the basis and the ack and re-flags purely by derivation — D-SCHED-16's "never a flag to clear" principle is preserved, and `schedule_qty_basis` remains trigger-owned with no second writer. Handler mirrors the lot-split acknowledgment shape (direct update + audit + refetch, non-blocking, busy state on the button). Unmerging back to the original basis also clears, ack or no ack.
**Why:** A merge that lands after scheduling can be deliberately absorbed by the existing slot (the added member fits the machine's real rate), and until now the amber notice for that decision could only be silenced by opening Adjust End Date and re-saving — a fake reschedule to record "I saw it." The ack records the decision honestly (who, when, at what target) without touching the schedule, following the D-JOBMERGE-04 precedent of acknowledgments that participate in staleness derivation rather than boolean flags.
**Deploy:** SQL on TEST → this round on TEST → verify → SQL on PROD → deploy.
**Files:** src/lib/jobMerge.js, src/pages/Schedule.jsx; Docs/migrations/2026-09-04_D-SCHED-24_ack_columns.sql (run by Matt, not CC).

## Pricing Portal — Sprint 11 (2026-09-05)

Discovery closed 2026-09-03; decisions locked in `Docs/Implementation_Plans/S11_Implementation_Plan.md` §4 and reproduced here verbatim. D-PRICE-26 records what Batch A actually built on the bridge side.

### D-PRICE-01 — Source of truth (2026-09-05)
The portal's active price book. The Excel guide is retired after cutover; Rev 81 is the seed.

### D-PRICE-02 — Price model (2026-09-05)
`unit price = Each × multiplier`. Each stored to **3 dp** (`ROUND(x,3)`). Displayed/quoted prices rounded half-up to 2 dp at use time.

### D-PRICE-03 — Columns (2026-09-05)
Quantity breaks 100 / 300 / 500 — everyone. Tier 1 / 2 / 3 / **Premier** — customer qualification, no quantity trigger. The quantities printed in the guide's tier headers are ignored.

### D-PRICE-04 — Tier vs break (2026-09-05)
A tiered customer pays the tier price at any quantity (tier multipliers are always below the 500-break in every rule). Non-tiered customers get the largest break their quantity reaches.

### D-PRICE-05 — Premier (2026-09-05)
One standard: **Premier = Tier 3 × 0.97** on parts flagged `has_premier` (the 66 that carry the column today). A Premier customer buying an unflagged part pays Tier 3.

### D-PRICE-06 — Customer-part specials (2026-09-05)
SK201-2…-6 (PinAir, 83.3% of T3) and SK40R17-1/-1E/-2 (Air Tractor, 82.4% of T3) become `price_exceptions` keyed on customer × part as a **percent of Tier 3**, so they move with the Each. Other Premier customers on those parts get the 97% standard.

### D-PRICE-07 — Rule table (2026-09-05)
F Tier 3 = 0.73. K → D. N → C. P retired; its 4 parts → A with ladder `each_t1_t2` (no breaks, no Tier 3). Rule table becomes 7 columns (incl. Premier as the global 0.97).

### D-PRICE-08 — Ladders (2026-09-05)
Per section, loaded as decided: standard (100/300/500/T1/T2/T3), tools (5/10/25), platemount (10/50/T1/T2/T3-Dealer), etc. Kit/set header ladders ignored. Every ladder is data (`price_ladders`), never schema.

### D-PRICE-09 — Kits / sets (2026-09-05)
17 SET rows + Cloc 2000 Kit = Σ component prices per column, computed live from `kit_components`. No stored kit price.

### D-PRICE-10 — Range rows (2026-09-05)
Kept as the price holder; expanded on load to the Fishbowl SKUs that exist (962). Ranges with no product (30) load nothing. Range syntax is never parsed at runtime.

### D-PRICE-11 — Duplicates / overrides (2026-09-05)
Duplicate part rows: higher Each wins. Hand-typed break cells ignored. Each-as-formula → its value. Each = 0 row dropped.

### D-PRICE-12 — Unpriced parts (2026-09-05)
76 guide rows without an Each and 120 resale items → `status = 'no_price'`; visible in Catalog as "No pricing available", excluded from sheets/quotes.

### D-PRICE-13 — Resale section (2026-09-05)
1,170 Fishbowl products sold since Nov-23 and not on the guide → section **Resale Items**: Each = Fishbowl `product.price`, ladder `none` (no breaks, no tiers, no Premier). Matt's proposal-sheet edits applied (8 removed, 3 priced, 3 aliased). Resale Eaches are **not** uplifted on Oct 1 (assumption — see §11).

### D-PRICE-14 — DFAR (2026-09-05)
`R` and blank → `N`.

### D-PRICE-15 — Oct 1 (2026-09-05)
Rev 82 = clone of Rev 81 with every catalog Each × 1.15 (3 dp), `effective_from = 2026-10-01`, status `scheduled`. Price resolution is by date, so activation is automatic and price sheets "as of Oct 1" can be produced in September.

### D-PRICE-16 — Book lifecycle (2026-09-05)
`draft → scheduled → active → superseded`. Only one book can be effective for a date. Edits only on drafts; a scheduled book reverts to draft to edit.

### D-PRICE-17 — Access (2026-09-05)
Edit (books, items, rules, tiers, exceptions): **admin** only. View everything (incl. tiers, history, sheets): admin, `customer_service` (April, Christy, Sawyer, Peyton), president, viewer. Office login only — no PIN, no kiosk JWT. Gate in `roles.js` (`canViewPricing`, `canEditPricing`) and in every RPC (`_pricing_gate`).

### D-PRICE-18 — Customer key (2026-09-05)
Fishbowl is the customer master. Tiers and exceptions key on `fb_customer_id`; SkyNet `customers.id` is linked where the Order Queue linker already set it.

### D-PRICE-19 — Purchase history (2026-09-05)
`fb_so_history_lines` = every SO line since 2023-11-27 (all statuses except Estimate/Void), refreshed nightly by `dateLastModified` + on-demand; open SOs continue to come from `fb_sales_order_lines`. `v_customer_purchases` unions both.

### D-PRICE-20 — Salesperson (2026-09-05)
`fb_customers.salesman` (Fishbowl username) → SkyNet profile via `fb_users`/email; rep tags in names are display-stripped (`name_clean`) until the Fishbowl rename runs.
...
### D-PRICE-26 — Bridge v1.3: customers, products and SO history mirrors (2026-09-05)
**What:** Three read-only pollers added to the Fishbowl bridge (v1.2.0 → v1.3.0), all sharing the existing cycle's single Fishbowl session so no second session is ever opened (D-FB-37). **customers** every `POLL_CUSTOMERS_SEC` (900 s): full backfill when `fb_sync_state.last_customers_at` is NULL, then `customer.dateLastModified > last_customers_at − 1 hour`, joined to `sysuser` for the salesman username and `paymentterms` for the terms, with account groups mapped on by `customer.accountId` through `accountgrouprelation(id, accountId, groupId)` ⋈ `accountgroup`. **products** nightly at `PRODUCTS_NIGHTLY_AT` (02:10 local): the whole `product` ⋈ `part` table, 10,980 rows, no incremental mode. **so_history** nightly at `HISTORY_NIGHTLY_AT` (02:20 local): product lines (`soitem.typeId` 10 Sale / 30 Misc Sale) of every SO that is not an Estimate (10), Voided (80), Cancelled (85) or Expired (90) — open orders included so the table is complete on its own — paged `ORDER BY so.dateLastModified, soitem.id LIMIT 2000` from `fb_sync_state.history_cursor` (`HISTORY_BACKFILL_FROM=2023-11-27` when there is none), ~35k lines over ~20 pages on the backfill. All three write in 500-row batches through `_pricing_gate('integration')` RPCs — `fb_upsert_customers` / `fb_upsert_products` / `fb_upsert_so_history` — which stamp their own `fb_sync_state.last_*_at`; the history RPC also stores each page's max `dateLastModified` as the new cursor when that page's last batch lands (a partial batch passes NULL and leaves the cursor), so an interrupted backfill resumes instead of restarting. `name_clean` is computed server-side by `_fb_clean_name()` (strips a trailing `/AB /CE /PM /SG /HC` rep tag) and `fb_customers.customer_id` self-links to SkyNet `customers` by Fishbowl customer number. `--backfill` runs all three once from scratch and exits (`npm run backfill:pricing`); the SO backfill (`npm run backfill`) is unchanged. Nightly means "today's HH:MM has passed and the last run was before it", derived from the `fb_sync_state` timestamps rather than an in-process timer, so a restart cannot double-run a nightly job and a bridge that was down at 02:20 catches up when it returns. `paymentterms` and `accountgroup` are the two table names not confirmed on 25.9: either read failing degrades the poller for the life of the process with one logged warning — terms sent as NULL, the `groups` key omitted entirely so the RPC keeps the stored array — rather than guessing column names. A poll that finds nothing still calls its RPC with an empty payload, so the three ages the portal shows mean "last polled", not "last time anything changed"; and a pricing failure never stops the SO tail — it is logged and the three pollers stand down for 15 minutes, which also stops a nightly job whose clock was not stamped from hot-looping a broken query every 20 s.
...
**Why:** The RPC is the single authority for a price; the client mirror exists only so a grid does not need a 23k-row RPC call to draw one section (§5.4 of the plan). They are deliberately shown side by side on Lookup rather than hidden behind a test, so a divergence is caught by whoever is using the page — and the rule is that a disagreement means the mirror is wrong, never the RPC.
**Deviation from the round brief:** the six files were delivered whole and created verbatim, except that eleven `// eslint-disable-next-line react-hooks/set-state-in-effect` comments were added (the codebase's existing convention for that rule, as in CreateWorkOrderModal.jsx and StockRequestModal.jsx), `PriceCatalog`'s `sections` was wrapped in `useMemo` as the deps rule instructs, and one `exhaustive-deps` directive in PriceCustomers.jsx that the current plugin no longer needs was removed. No logic changed; the brief's own VERIFY step requires eslint clean on these paths, and the repo-wide lint count is unchanged at 35 errors / 38 warnings.
**Files:** src/pages/Pricing.jsx, src/components/pricing/{PricingTypeaheads,PriceLookup,PriceCatalog,PriceCustomers}.jsx, src/lib/pricing.js (all new), src/lib/roles.js, src/config.js, src/App.jsx; Docs/migrations/2026-09-08_pricing_b.sql (the `last_paid` view fix, applied by Matt, not CC).

### D-PRICE-28 — Quote Builder, statistics, exclusions, drop-ship line type (2026-09-09)
**What:** Lookup is renamed **Quote Builder** and is the portal's default tab. `pricing_get_price` stays authoritative, but its answer is now presented as the ★ **recommended** column rather than the only one: every column chip on the ladder strip is clickable, and choosing another records the override on the line (`is_override`, `recommended_col`, `recommended_price`) with **no reason required** (Matt, 2026-09-04). **Add to Quote** builds a browser-local draft under `skynet.pricing.quote_draft.<uid>` — qty edit, remove, clear, subtotal, override count — shaped exactly like the coming `quote_lines` table so nothing is re-keyed when Batch C gives quotes a number and a PDF (reordered ahead of the price sheet at Matt's request). Adding a line for a different customer starts a new draft behind an amber notice. `pricing_excluded_products` (54 rows: the 8 resale removals plus non-inventory part types — CUSTOMER NOTES, CATALOG, freight, labor) is honoured by the view, both statistics RPCs and the Fishbowl-product typeahead (`loadExcludedKeys`, cached per session). `pricing_top_customers` powers a top-10-by-trailing-12-month-revenue landing panel on Customers (click a row to open it); `pricing_part_customers` powers "Who buys this part" under the Quote Builder result. The customer's last SO lines of the part gain column headers and a status pill (line status, falling back to SO status, labelled from `lib/fishbowl.js`). The Catalog sidebar shows full section names that wrap, with a drag handle (220–720 px) and a collapse button, both remembered in `skynet.pricing.catalog_sidebar`. Sets now resolve their components across the whole book via `loadItemsByKeys`, not just the current section — in the Catalog grid and the Quote Builder ladder strip alike.
**Correction:** the SO-history poller filtered `si.typeId IN (10, 30)`. **30 is Discount %, not drop ship** — drop ship is 12, and 10/12 is the pair D-FB-08 already calls `PRODUCT_LINE_TYPES` (see `FB_LINE_TYPE` in src/lib/fishbowl.js). The Batch A brief and D-PRICE-26 both carried the error, so discount lines were being counted as purchases and drop-ship lines were missing entirely. The filter is now `(10, 12)`; the migration purged the type-30 rows and reset `fb_sync_state.history_cursor`, so the next `npm run backfill:pricing` re-reads history from 2023-11-27. Note the literal lived in `tools/fishbowl-bridge/src/queries.mjs` (`q.soHistory`), not `pricing.mjs` as the round brief assumed — the SQL has been in queries.mjs since Batch A.
**Why:** Matt's TEST walkthrough of Batch B on 2026-09-04, six items. The override model is the substantive one: reps priced from the Excel guide by eye and sometimes quoted a different column deliberately, so the portal has to permit that and record it rather than force the book's answer — the recommendation is advice with an audit trail, not a gate.
**Deviation from the round brief:** the five files were replaced whole as delivered, except that five `// eslint-disable-next-line react-hooks/set-state-in-effect` comments were added (the codebase convention, as in Batch B), `PriceCatalog`'s `sections` was wrapped in `useMemo`, the now-unused `todayIso` import was dropped from QuoteBuilder.jsx (the prop of the same name shadows it, so the import was dead and tripped `no-unused-vars`), and the unused `exhaustive-deps` directive in PriceCustomers.jsx was again omitted — same three classes recorded under D-PRICE-27. No logic changed; repo-wide lint is unchanged at 35 errors / 38 warnings.
**Files:** src/components/pricing/QuoteBuilder.jsx (new — replaces PriceLookup.jsx, deleted), PriceCatalog.jsx, PriceCustomers.jsx, src/pages/Pricing.jsx, src/lib/pricing.js, tools/fishbowl-bridge/src/queries.mjs (the line-type fix), Docs/Fishbowl_Data_Context.md (line-type correction), Docs/migrations/2026-09-09_pricing_b1.sql (applied by Matt, not CC).

### D-PRICE-29 — Customer price lists; specials from lists; part images (2026-09-10)
**What:** `price_lists` / `price_list_lines` with `PL-YYMM-NNNN` numbering (`pricing_counters` + `pricing_next_number`, shared with quotes in C2). Reps — view roles, not just admin — open **Create Price List** from a customer card and get every part that customer has bought, priced at their level on a chosen date (`pricing_customer_sheet`), with Last paid / Each (list) / Book price / **Your price** columns. They may add parts by typeahead, drop rows, change prices and add notes. **Save & issue** numbers the list, supersedes that customer's previous issued list, and records each changed price as a **fixed** customer-part special (`price_exceptions.mode = 'fixed'`, note "from price list PL-…") unless the list opts out of it — the "Record changed prices as customer specials" toggle, default on. PDF (pdf-lib) and XLSX (SheetJS) are generated client-side **from the saved rows**, not from the builder's state, so a re-download months later is what the customer actually received; the customer card lists every issued list with a re-download button. Email send is deliberately out of scope this round — reps attach the PDF themselves until there is an app-level SES sender. Also: the Catalog grid gains a thumbnail column and a section header image, and the Quote Builder shows the part image beside the result, from `price_images` (part key, or the range row's key for range-derived items) over the public `pricing-images` bucket — 198 files extracted from the Rev 81 workbook's drawing anchors (149 part, 49 section). Two fixes from Matt's review: a walk-in quote draft compared `undefined` with `null` and so started a new draft on every Add (now `sameCustomer` normalises both sides with `?? null`), and the Top-10 panel drops its Rep column.
**Why:** Matt, 2026-09-04 — a price list that has gone out to a customer is a commitment, so the portal has to remember what was sent and honour it afterwards; recording changed prices as specials is what makes the next quote and the next list agree with the paper the customer is holding. Regenerating documents from the saved rows rather than from the screen is the same principle: the artifact of record is the row, not the render.
**Known risk — PDF text encoding:** pdf-lib's StandardFonts use WinAnsi and **throw** on any code point outside it, and `dl('pdf')` has no try/catch, so a bad character means the rep clicks PDF and silently gets nothing. Verified offline against the real `buildPriceListPdf`: cp1252 text is safe (®, °, ½, smart quotes, em dash, ellipsis, bullet all render), but U+2011 non-breaking hyphen, CJK and emoji all throw — and a non-breaking hyphen is exactly what a Word/Outlook paste into the Notes field produces. `src/lib/certPackagePdf.js` already solved this with a `safe()` sanitiser written for the same reason. Not applied here because priceListDoc.js was delivered whole; the fix for C2 is to run `part_number` / `description` / `customer_name` / `notes` through that sanitiser and wrap the download handlers in try/catch. Pagination itself is sound — verified 3 rows → 1 page, 40 → 2, 120 → 6.
**Deviation from the round brief:** the seven files were created as delivered except that `PriceCatalog`'s section-image lookup read `meta?.sections` inside an effect that did not depend on it (a real `exhaustive-deps` warning); it now uses the already-memoised `sections` and lists it in the deps. No behaviour change. The unused `exhaustive-deps` directive in PriceCustomers.jsx stays omitted, as in D-PRICE-27/28. `pdf-lib` and `xlsx` were already dependencies (certPackagePdf.js uses both), so nothing was installed. Repo-wide lint unchanged at 35 errors / 38 warnings.
**Files:** src/lib/priceListDoc.js (new), src/components/pricing/PriceListBuilder.jsx (new), PriceCustomers.jsx, PriceCatalog.jsx, QuoteBuilder.jsx, src/pages/Pricing.jsx, src/lib/pricing.js, Docs/migrations/2026-09-10_pricing_c1.sql (applied by Matt, not CC — with the `pricing-images` bucket upload).

### D-PRICE-30 — Catalog pictures: gallery below the grid + lightbox (2026-09-10)
**What:** C1 rendered pictures in two different places — a thumbnail column on rows that had a part image, and the section drawing above the table — so a section looked different depending on which pictures existed. Both are removed. Every picture for a section now appears in one place, **below the rows**: "Pictures in this section — click to enlarge", a wrap of 176×144 white cards captioned with the part number. The section drawing is the first card (captioned with the section name), then one card per pictured part, de-duplicated by image URL so parts sharing a picture — a range — collapse to a single card captioned with the range string. Clicking any card opens `ImageLightbox`: full-screen, Esc / backdrop / click-the-picture to close, ← → keys and on-screen arrows to move, and "n / N" beside the caption. The Quote Builder's part image beside the result became a button that opens the same lightbox with a single image. Note the gallery is built from the section's full item list, not the filtered view — typing in "filter this section" narrows the grid but not the pictures, which matches the "Pictures in this section" label.
**Why:** Matt's C1 review on TEST — uniform placement beats per-row thumbnails when only some rows have a picture, and reps need to see the drawing large enough to read.
**Deviation from the round brief:** the delivered `PriceCatalog.jsx` reverted the three lint fixes carried since B.1/C1, so they were re-applied to keep eslint at its zero baseline for this module: `sections` wrapped in `useMemo`, the section-load effect using that memo and listing it in its deps rather than reading `meta?.sections`, and three `// eslint-disable-next-line react-hooks/set-state-in-effect` comments (the codebase convention). No behaviour change. Repo-wide lint unchanged at 35 errors / 38 warnings.
**Files:** src/components/pricing/ImageLightbox.jsx (new), PriceCatalog.jsx, QuoteBuilder.jsx. No SQL, no new dependency — the pictures are the `pricing-images` bucket and `price_images` rows from D-PRICE-29.

### D-PRICE-31 — Admin picture management in the Catalog (2026-09-11)
**What:** The section gallery gains admin controls: a target picker — **Section picture** or any part in the section, with "(replace)" beside parts that already have one — and **Add picture** (JPEG/PNG/WebP). The file uploads browser-side straight to the public `pricing-images` bucket under a unique `parts/<KEY>_<ts>.<ext>` or `sections/<source_row>_<ts>.<ext>` name, then `pricing_upsert_image` registers it; if the RPC fails the just-uploaded file is removed again, so a rejected write cannot leave an orphan in the bucket. Each card gets a hover **×** that calls `pricing_delete_image` and then deletes the file. A part holds one picture (adding replaces it); a section may hold several, all shown first in the gallery with "· section" on the caption. `loadPartImages` now returns `{ id, path, src }` per key and `loadSectionImage` becomes `loadSectionImages` returning an array — Quote Builder reads `?.src` for the new shape. Non-admins see the gallery exactly as before: no picker, no Add, no ×, and the RPCs and Storage policies are admin-only server-side, so a stray control would fail rather than leak.
**Why:** Matt, 2026-09-04, once the gallery landed — the Rev 81 extraction got 198 pictures but the book keeps changing, and re-running an extraction is not how a picture should get fixed.
**Deviation — the round brief asked for the `set-state-in-effect` directives to be re-applied; they are deliberately absent.** Carried since B.1, they were verified dead on this version of the file: removing all three leaves eslint reporting nothing, while keeping them produces three "unused eslint-disable directive" warnings. The memoised `sections` was tested the same way and IS still load-bearing (two `exhaustive-deps` warnings without it), so it stays. The intent of the instruction — keep the module lint-clean — is met at 0 problems; following its letter would have added the warnings it was meant to prevent.
**Also fixed:** `addTarget` was not reset when the section changed, so an admin who picked a part, switched sections and then clicked Add picture would silently attach the image to a part in the *previous* section — a wrong write with no visible error. The section button already cleared search, filter and lightbox; it now clears the picture target and any upload error too. One line, in the handler that already existed for exactly this.
**Files:** src/lib/pricing.js, src/components/pricing/PriceCatalog.jsx, QuoteBuilder.jsx, src/pages/Pricing.jsx, Docs/migrations/2026-09-11_pricing_c1_2.sql (applied by Matt, not CC).

### D-PRICE-32 — Saved quotes (2026-09-12)
**What:** `quotes` / `quote_lines`. View roles — not just admin — save a Quote Builder draft as `Q-YYMM-NNNN` with every line's unit price, quantity, chosen column, the book's recommendation and the override flag frozen onto the quote; `valid_until` = issue date + 14 days, the D-PRICE-23 lock, so those prices stand for that customer through that date whatever the book does in between. The draft panel gains the header the document needs: walk-in customer name (when no customer is picked), contact, email, customer PO/RFQ, payment terms and printed notes. Save issues the number, generates the PDF from the *saved* rows and clears the draft, leaving a green "Saved Q-… · valid until …" note. A Recent quotes card (last 12, all customers) carries a status pill, subtotal, PDF, Reopen, Won and Lost; Reopen loads the lines into a new draft and saving issues a fresh number and supersedes the original; Won prompts for the Fishbowl SO number. Statuses are issued / won / lost / cancelled / superseded, with "Expired" derived from `valid_until` rather than stored. The customer card lists that customer's quotes above their price lists. Quotes deliberately do **not** create customer specials — only price lists do (D-PRICE-29): a quote is an offer with an expiry, a price list is a standing commitment.
**Why:** Matt's 2026-09-04 reorder put quotes ahead of the price sheet, and the 14-day lock came out of discovery. Regenerating the PDF from the saved rows rather than the screen is the same rule the price lists follow — the row is the artifact of record.
**Fixed in the delivered code — quote totals printed on one line.** `buildQuotePdf`'s `right()` helper closes over the outer `y`, and the Subtotal / Sales Tax / Total loop stepped only its *boxes* down (via a local `yy`) while never moving `y` — so all three labels and all three amounts drew on a single baseline under three stacked boxes, on every quote. The three rows are now anchored to `yy` with the same fonts, sizes and x-anchors as delivered, and `y` is left alone for the terms block that follows. Unconditional, would have shown on the first PDF.
**Known risk carried from D-PRICE-29, now sharper: PDF text encoding.** pdf-lib's StandardFonts are WinAnsi and throw on anything outside it. Verified offline against the real `buildQuotePdf`: ®, °, smart quotes, em dash, en dash, ellipsis and bullet all render, but U+2011 non-breaking hyphen, CJK and emoji throw — and this round adds five rep-typed fields (contact, email, PO, payment terms, notes) where a Word or Outlook paste plausibly carries one. The failure is worse here than on price lists: `saveTheQuote` runs `pricing_save_quote` **before** building the PDF, so a throw leaves the quote saved and numbered while the rep sees an error over a still-full draft — pressing Save again issues a second number for the same quote. Not fixed here: the remedy touches customer-facing output (silently substituting characters via the `safe()` sanitiser in src/lib/certPackagePdf.js) or the save ordering, and both are Matt's call. Recommended for the next round: sanitise in quoteDoc.js / priceListDoc.js and move the PDF step out of the path that gates `setDraft(null)`. Pagination itself is sound — verified 2 lines → 1 page, 40 → 4, 150 → 11.
**Files:** src/lib/quoteDoc.js (new), src/lib/pricing.js, src/components/pricing/QuoteBuilder.jsx, PriceCustomers.jsx, Docs/migrations/2026-09-12_pricing_c2.sql (applied by Matt, not CC).

### D-PRICE-33 — PDF branding, WinAnsi-safe text, save-before-PDF (2026-09-12)
**What:** Quote and price-list PDFs carry the Skybolt logo and brand colours (red #B8222E, grey #6D6E70) via `src/lib/pdfText.js`; all drawn strings pass `safe()` so rep-typed text cannot crash rendering; a quote is committed (draft cleared, list refreshed) before its PDF renders so a PDF failure cannot cause a duplicate number.
**Why:** Matt's review of the first quote PDF 2026-09-04 and CC's C2 risk note.
**Verified offline against the real builders:** the four inputs that threw under D-PRICE-32 — U+2011 non-breaking hyphen, NBSP, CJK, emoji — now all render, with the hyphen printing as `-` and unencodable characters as `?`, while cp1252 (®, °) survives unchanged and newline/tab are preserved. The logo embeds in both documents, and with the logo missing (fetch rejecting, as a rename would) both still render — `drawLetterhead` swallows the failure and the letterhead falls back to text. Pagination unaffected: quote 2 lines → 1 page, 60 → 4+; price list 120 → 3+.
**Deviation:** `safe()` is the pattern from `src/lib/certPackagePdf.js`, including its final `[^\x09\x0A\x0D\x20-\xFF]` guard, which trips `no-control-regex` — that file already carries the same error as one of the repo's pre-existing 35. Rather than add a 36th, the line has an `eslint-disable-next-line no-control-regex` and a comment saying the control characters are matched deliberately, keeping the pricing module at zero problems and the repo at its baseline.
**Files:** src/lib/pdfText.js (new), public/skybolt_logo.jpg (added by Matt), src/lib/quoteDoc.js, src/lib/priceListDoc.js, src/components/pricing/QuoteBuilder.jsx.

### D-PRICE-34 — Price Books editor (2026-09-13)
**What:** The Price Books tab becomes the admin editor over the Batch A RPCs — clone with uplift, draft-only edits of items / sections / rules, bulk uplift, diff between any two books, schedule / unschedule / publish, and the Fishbowl Products CSV export (D-PRICE-22 interim write-back); `pricing_roll_books()` runs on portal load so status follows the date. Ladder columns remain SQL-maintained. Everyone who can open the portal sees the list, the sections/items and the diff read-only; only admin gets Clone, the inputs and Save/Remove — and the RPCs enforce the same gate, so a stray control would fail rather than leak. Editing is confined to drafts by status: a scheduled book must be unscheduled first (D-PRICE-16), and the active book is read-only with a note pointing at Clone.
**Why:** Last piece of the S11 plan (§8.4) before the PROD cutover; the Oct 1 book was already seeded and scheduled, and this gives Matt the tools to change it without SQL.
**Verified offline** (both are pure functions, exercised against the real source): `diffBooks` reports changed / added / removed correctly, ignores unchanged rows, treats null → null as no change, and catches rule / ladder / status / Premier moves as well as price moves — the walkthrough figure 5.275 → 6.100 comes out at **+15.6%**, and on a +15% clone the resale rows correctly do not appear (D-PRICE-13). `productsCsv` emits the header, only `priced` rows with a non-null Each, 2 dp, CRLF line endings, and RFC-4180 quoting for part numbers containing a comma or a quote.
...
**SUPERSEDED by** D-PRICE-39 (2026-09-08): the section control now sets a total % over the in-effect book via `pricing_set_section_vs_base`; the compounding "Uplift section" path was never used on PROD.

### D-PRICE-39 — Section repricing anchored to the in-effect book (2026-09-08)
**What:** The Sections & items toolbar's **Uplift section** (D-PRICE-38) is replaced by **Set section**: the input is the section's total % over the in-effect book (placeholder = where it sits today), and the write is `pricing_set_section_vs_base(p_book, p_section, p_base, p_total_pct)`, which sets every priced row in the section to `round(base_each × (1 + pct), 3)` matched on `part_key` against the base book (higher Each wins on a duplicate key, D-PRICE-11); rows with no priced base row are left unchanged and counted as `unmatched`. Gates as `pricing_uplift_book` (edit roles, draft only) plus: base ≠ book, section belongs to the book, section kind = catalog. Returns `{updated, unmatched}`; the confirm quotes the current and target position, the ×factor and the skipped count, and the flash repeats the counts. The control is hidden until the in-effect book's items have loaded. **Uplift all** is unchanged — a clone's Each is exactly base × (1 + uplift), so the two controls agree.
...
### D-PRICE-41 — Whitespace removed from price-book part numbers (2026-09-08)
**What:** Rev 81 and Rev 82 rows whose `part_number` carried internal whitespace from the Excel extraction (`SK2601- 1SFW`) were set to the stripped form, on both books, in one guarded one-shot (`Docs/migrations/2026-09-08_part_number_whitespace_fix.sql` Blocks 0–1/3 and `…_block2_amended.sql`). Rule, not a blanket strip: Fishbowl carries 551 product numbers with real internal spaces and the Resale section is made of them, so catalog rows strip unless the spaced form is itself an exact Fishbowl product number, and resale rows strip only when the stripped form is the Fishbowl number and the spaced form is not. 7 catalog rows per book keep their spaces because Fishbowl names them that way (`SK2018 ZLOC SET`, `SK-R500 SET`, `SKN4-4S SET`, `SK220-2S w/o Nut`, `SK214-16S Rev D - Short`, `SK-RVC1 14`, `SK-RVC1 19`), as do 5 resale rows (`AN380-2-2 (100) piece bag`, `MFG CERTS`, `NADCAP CERT`, `PLATING FEE`, `SK214-16S - Standard`). Override: `SK-N114 -2S/-3S/-4S` stripped although a spaced Fishbowl product exists, because that product is the retired one ("(OLD DO NOT USE)" siblings) and all 2025–26 sales are on `SK-N114-2S/-3S/-4S`. `price_kit_components.component_part_number` stripped likewise; `price_exceptions.part_number` aligned to the active book's row by `part_key`. `quote_lines` / `price_list_lines` untouched — issued documents stay as issued. Verified TEST and PROD identical: catalog still spaced 14 (the 7 × 2 keeps), resale 10, kit components 0, key mismatches 0, exceptions unaligned 0.
**Why:** Found by Matt in the Catalog. Pricing was never affected — `part_key` / `component_key` were already whitespace-free, so search, kit sums, exceptions and the D-PRICE-39 base join all worked — but the display, the PDFs and above all the Fishbowl Products CSV (D-PRICE-22) carried the spaced number, and Fishbowl matches on the exact product number, so those parts would not have updated on Oct 1.
**Schema correction:** `price_items.part_key` and `price_kit_components.component_key` are GENERATED columns (the first fix attempt failed with 428C9 assigning them); `Docs/Supabase SQL Database.txt` records them as DEFAULT expressions and should be refreshed.
**Fishbowl follow-up (backlog D):** inactivate the retired spaced `SK-N114 -2S/-3S/-4S` products with the never-sold batch so a future re-seed does not re-import them.
**Files:** Docs/migrations/2026-09-08_part_number_whitespace_fix.sql, Docs/migrations/2026-09-08_part_number_whitespace_fix_block2_amended.sql. No code.

### D-LATEBATCH-02 — Late batches for outsourcing-routed jobs; vendor-return step guard (2026-09-09)
**What:** Late Parts eligibility (Finishing station) widens from manufacturing_complete / pending_tco / ready_for_assembly to also admit ready_for_outsourcing and at_external_vendor, and the window widens from 5 to 14 days. The status revert is now status-aware: pending_tco / ready_for_assembly still drop back to manufacturing_complete (ComplianceReview.handleApproveBatch advances only from there); the two outsourcing statuses keep their status, because the job is already past the point approval would advance it to — the approved late batch surfaces on the Outsourcing screen as its own send (that list is keyed on approved finishing_sends with an external step and no outbound_sends row, and does not exclude these statuses). Modal copy, confirmation alerts, and the audit's status_reverted flag follow the same predicate. Separately, OutsourcedJobs' vendor-return handler gains a guard: a routing step is marked complete (and the job advanced to pending_tco / ready_for_assembly) only when every returned send is back AND no non-rejected finishing batch for the job lacks an outbound send for that step — a batch still in finishing, awaiting compliance, or approved-but-unsent holds the step open. Complete (TCO closed) remains excluded from Late Parts; custody has transferred.
**Why:** J-000206 (SK212-10AB, external Plating, batches A/B at Electrolab, 698/850) was closed at the kiosk before its final pieces were batched and did not appear in Late Parts — the D-LATEBATCH-01 status list was written for the non-outsourced path. The guard closes the race the widening exposes: without it, batches A/B returning while late batch C is still upstream would mark the Plating step complete and advance the job to pending_tco, stranding C approved-but-never-plated and invisible to the Outsourcing list. The guard is also correct on its own — the same race exists today for any final batch still in compliance when earlier batches return. Merging Late Parts into Recent Completions was considered and rejected: Recent Completions is keyed on finishing batches (one row per FLN), so the most common premature close — a job with zero batches — would never appear there, and multi-batch jobs would appear once per batch; the panels answer different questions (what finishing produced vs. which jobs can still receive a batch).
**Deploy:** TEST first; frontend-only, no migration.
**Files:** src/pages/Finishing.jsx, src/components/OutsourcedJobs.jsx.

**Amendment (2026-09-09):** The upstream-batch guard is also applied inside handleReturnGroup, the consolidated-lot return path, which runs its own copy of the step-complete → advance cascade; CC flagged the gap on the first round. Behavior is identical: a non-rejected finishing batch without an outbound send on the step holds the step open (`continue`), and the step/job advance runs on the final return. The guard's finishing_sends read throws into handleReturnGroup's existing try/catch on error, like the rest of that loop's reads.

### D-DOWN-02 — Kiosk DOWN strip renders on any DOWN signal, including a bare machine flag (2026-09-09)
**What:** The kiosk's Machine DOWN strip (and its Machine Ready control) now renders when `machine.status === 'down'` OR open downtime rows exist, instead of on open rows alone. With no open row the strip summarises the flag's own `status_reason` / `status_updated_at`, the Details block states plainly that the machine is flagged on status only, and the Machine Ready modal copy no longer promises to close "0 open downtime records". No handler change: handleClearMachineDown already tolerates zero rows and already refuses to clear the flag while an unplanned-maintenance job is open (D-DOWN-01), so the existing guard still protects the DTU path. Frontend-only.
**Why:** Ganesh 1 sat DOWN with no way back into service: flag set, zero open downtime rows, no DTU job in its queue. D-DOWN-01 established that the flag, open downtime rows, and an open unplanned-maintenance job are three independent signals — but left the only clear control keyed on one of them. Any path that sets or leaves the flag without an open row strands the machine: the kiosk downtime modal flags DOWN even when the logged row was given an end time; a DTU job cancelled from Mainframe (rather than completed at the kiosk or via Command) never clears the flag; and every prior SQL fix cleared an instance. Rule going forward: if any signal says DOWN, the kiosk offers the way out, guarded only by an open unplanned-maintenance job.
**Files:** src/pages/Kiosk.jsx.

### D-CODATE-01 — Three dates per customer-order line: entered, SkyNet target, Fishbowl due (2026-09-10)
**What:** A read-time view `v_co_line_dates` (security_invoker; migration 2026-09-10_D-CODATE-01_co_line_dates.sql, with an immutable `add_business_days(date, int)` helper) derives, per CO line: `entered_on` — the Fishbowl SO creation date (`fb_sales_orders.fb_date_created`, America/New_York, earliest over the line's linked Fishbowl lines) for Fishbowl-sourced lines, else the SkyNet line's `created_at`; `target_date` = entered_on + 45 business days (Mon–Fri; no holiday table yet); `fb_due_date` = Fishbowl's live effective due (earliest linked `effective_due_date`), else the line's own `due_date` for manual COs, with `fb_due_is_default` carried through; `scheduled_finish` = latest `scheduled_end` across non-cancelled, non-maintenance jobs on the line's actively allocated WOs, plus `has_unscheduled_jobs`. Nothing is stored and nothing is added to the bridge, ingest, or `fb_convert_to_co`; `customer_order_lines.due_date` keeps its meaning. `getCOLineDates` (chunked by 150 ids) attaches the fields in `getAllOpenCOLines` and the Orders loader. Demand: the group header and the "Earliest target ↑" sort key on the earliest target (Fishbowl due beneath, muted); the line table's Entered column now reads the view's entered_on (fixing a UTC-midnight day slip in the old `new Date(created_at)` render) and Due splits into Target and FB Due. Orders: the CO row shows Target / FB Due; the line table shows Entered, Target, FB Due, Finish. Cues: FB Due amber when earlier than the target (the customer wants it sooner than the rule allows — a conversation, not a plan); `*` when Fishbowl never received a real date; Finish red when later than the target, "unsched." when allocated work has no slot, "+" when some of it does not.
**Why:** SkyNet had no due-date rule of its own — the only sources were Fishbowl's effective date (which defaults to the SO creation timestamp when the rep enters nothing, so orders arrive "due today") and the manual entry field. The shop's commitment is entered + 45 business days regardless of what Fishbowl says, Fishbowl's date can change on any sync, and the schedule's finish is a third thing; conflating them in one column made the Demand pool sort by a date nobody meant. Deriving at read time keeps all three honest without a second writer to any stored column; if a per-line negotiated target is ever needed, it becomes an override column layered over this rule.
**Deploy:** SQL on TEST → this round on TEST → verify → SQL on PROD → deploy.
**Files:** src/lib/customerOrders.js, src/pages/CustomerOrders.jsx; Docs/migrations/2026-09-10_D-CODATE-01_co_line_dates.sql (run by Matt, not CC).

### D-MACHINFO-01 — Sticky machine column in every scrolling view; machine hover card (2026-09-10)
**What:** The Command grid's row header (machine name / code / utilization) is now `sticky left-0` whenever the body can scroll sideways — day zoom AND the 2-/4-week windows (`windowDays > 7`, which set `min-w-max`) — instead of day zoom only, so the machine label stays in view while scrolling right. Hovering the row header shows a card with the machine's model, model year, max bar diameter, max bar length, type, location, and free-text capabilities. The data lives in four additive columns on `machines` (`model`, `model_year`, `max_bar_diameter_in`, `capabilities`; migration 2026-09-10_D-MACHINFO-01_machine_model.sql), seeded by machine name from the costing model's Machine Master sheet with NULL-only fills so hand edits survive re-runs; bar capacity comes from the model designation (SA-20 = 0.787", SA-32 / Gen-Turn 32 / Cyclone 32 = 1.260", Syncrex 25 = 0.984"), Mazak Multiplex capacities left for Matt. Maintained by SQL until a machines admin screen exists. The Schedule machines fetch already selects `*`, so no select-list change.
**Why:** In the 4-week view the machine label scrolled off with the timeline, leaving bars with no row identity; and the scheduler could not see from the grid which Nexturn is a 20 mm vs 32 mm machine when deciding whether a part in the pool can run there. Nexturn 3 (SA-32B) and Nexturn 7 (SA-32PYII) are the 32s; the rest are SA-20B.
**Deploy:** SQL on TEST → this round on TEST → verify → SQL on PROD → deploy.
**Files:** src/pages/Schedule.jsx; Docs/migrations/2026-09-10_D-MACHINFO-01_machine_model.sql (run by Matt, not CC).

**Amendment (2026-09-10, stacking context + default zoom):** The hover card rendered BEHIND job bars in the 2-/4-week windows and in day zoom — everywhere the cell is pinned. Cause: `sticky` + `z-index` makes the machine cell a stacking context, so the card's `z-50` is confined inside it and the cell's own `z-10` lost to the bars, which sit at `z-10` (multi-day) / `z-20` (highlighted) as later siblings in the same row. Week view was unaffected precisely because the cell is not pinned there, so the card competed at row level and won. Fixed by raising the pinned cell to `z-30`, above every bar layer; the day-zoom hour header moved `z-20 → z-40` (its corner `z-30 → z-50`) so the pinned column still scrolls UNDER it. An active resize drag draws its bar at `z-30` and can still cover the card — unreachable in practice, since the header cannot be hovered mid-drag. Separately, `windowDays` now initialises to 28: 4-Week is the view the scheduler works in, so the grid opens there instead of Week. Amends D-SCHED-ZOOM01's "zoom is session-only (resets to Week on reload)" on the default only — it is still session-only, and still resets on reload, now to 4-Week.

### D-CODATE-02 — SkyNet target in the schedule modal and Job Pool (2026-09-11)
**What:** A read-only view `v_wo_dates` (security_invoker; migration 2026-09-11_D-CODATE-02_wo_dates.sql) gives each work order the earliest SkyNet target, earliest Fishbowl due, and earliest entered date across its ACTIVE customer-order allocations (built on D-CODATE-01's `v_co_line_dates`); MTS work orders have no row. ScheduleJobModal fetches the row on open and its summary band now reads "Target <date>" with the Fishbowl due muted beside it (falling back to "Due <work_orders.due_date>" for MTS). D-DATE-03's late-schedule warning and confirm dialog now compare the scheduled finish against the target when one exists — the copy names which date it used. Job Pool cards show "Target:" with "FB" beneath; the pool fetch attaches the WO dates in one query. The modal fetches independently of the pool so it is correct from any opener.
**Why:** The band showed `work_orders.due_date`, which for Fishbowl-sourced orders is the Fishbowl effective date — often the entry date — so the scheduler was warned against a date nobody committed to and saw no sign of the real one (SK26CS6 / J-000273: "Due Sep 2" on a Sep 2 order). The commitment is entered + 45 business days; the plan should be judged against it.
**Deploy:** SQL on TEST → this round on TEST → verify → SQL on PROD → deploy.
**Files:** src/components/ScheduleJobModal.jsx, src/pages/Schedule.jsx; Docs/migrations/2026-09-11_D-CODATE-02_wo_dates.sql (run by Matt, not CC).

**Amendment (2026-09-11, D-CODATE-02b):** (1) Step 2's queue chips now show each queued job's commitment on the same basis as the job being scheduled — the SkyNet target when its WO has CO allocations ("Target <date>", Fishbowl due muted beside it), else "Due <work_orders.due_date>" — and D-SCHED-21's amber/red colouring compares target-to-target; the modal fetches v_wo_dates for every WO in the machine queues on open (chunked). (2) Schedule.jsx's `formatDate` now anchors bare YYYY-MM-DD values at local noon — it parsed them as UTC midnight, so the Job Pool cards (and the bars' "Due:") rendered a day early in Eastern time; CC caught it on the D-CODATE-02 round and the screenshot confirmed it (pool "Oct 20" vs modal "Oct 21"). (3) v_co_line_dates ignores Fishbowl and line due dates before 2000-01-01 (migration 2026-09-11_D-CODATE-02b_suspect_dates.sql) — WO-2606-0043's year-0007 Fishbowl typo had become its earliest due.

### D-RMF-09 — Config switch for "Extract from drawing" (2026-09-11)
**What:** `FEATURES.RM_DRAWING_EXTRACTION` in src/config.js gates the Needs-Data panel's per-part "Extract from drawing" button (D-RMF-05) and the panel's batched drawing lookup. Off (the shipped value): the button and its result card do not render, no job/part-document lookup runs for this purpose, and no extraction edge-function calls are made; the manual bar-length / material / bar-size fields and Save are unchanged. On: identical to before. Changing it is an edit + commit + deploy, like every other flag in that file.
**Why:** Matt wants the AI drawing read switched off in the forecast for now without losing the manual path or the code; a config flag keeps it one line to bring back.
**Files:** src/config.js, src/components/rmforecast/ExceptionsPanel.jsx.

### D-RPT-14 — Inventory Movement: interactive raw-material report by date range, with deterministic period summary (2026-09-12)
**What:** Registry row 'inventory-movement' (report_kind 'inventory_movement', universal view, export roles as part-history, source_object = RPC name — the column is NOT NULL even for interactive reports; upserted on slug, sort_order set on insert only as MAX+10 so a hand-tuned position survives a re-run). Two RPCs, both DROPped before create because the Sep 4 SQL Editor run may have left a different result type behind. report_inventory_movement(p_start,p_end) returns one row per raw-material movement (Received from material_receiving; Used from material_usage with quantity_used ≠ 0, job number cited; Adjustment from approved inventory_adjustment_requests), dated in America/New_York and filtered BETWEEN inclusive, valued at lot cost (price_per_bar, else weight × price/lb ÷ pieces; adjustments prefer price_per_bar_at_count; NULL cost carries no value and is counted). report_inventory_movement_summary(p_start,p_end) returns jsonb aggregates for the range and the immediately preceding equal-length range by selecting from the detail RPC twice in one query, tagged current/previous — one definition of a movement. Frontend InventoryMovementReport.jsx renders six cards, a templated narrative, a period-over-period table, receipts, draws by material, top jobs, a requeue watch (cumulative across both periods), data quality, a scorecard block, and the raw detail; CSV keeps registry columns verbatim (D-RPT-02); a Markdown export (clipboard, with a .md download beside it and as the fallback when the clipboard is blocked) reproduces the weekly Cowork story structure. D-RPT-12 integrity check: the component re-adds the detail rows by category and movement type and compares against the summary's current aggregates, plus net and row count; anything over $0.05 renders "INTEGRITY CHECK FAILED — do not quote this output" and disables both exports. No LLM anywhere; Uncle Bob not wired (envelope is flat-rowset, D-RPT-06/07) — follow-up.

Four decisions taken where the build prompt was silent or self-contradictory. (1) **Prior period is calendar arithmetic, not week alignment.** §4.2 specifies prior_end = p_start − 1 and prior_start = prior_end − (p_end − p_start), which is also what the registry explainer promises and the only rule that generalises to a 3-day or 45-day range; it makes the prior period for Mon Sep 7 – Fri Sep 11 be **Wed Sep 2 – Sun Sep 6**, so acceptance criterion 2's expectation that the previous column reproduces criterion 1 (Mon Aug 31 – Fri Sep 4) does not hold as written. The computed prior range is printed in the header, the narrative and the Markdown so nobody has to remember which window they are looking at. If the weekly story wants week-over-week, that is a business-day-aligned variant and a one-line change in the summary RPC. (2) `requeue` gained `current_bar_value` / `previous_bar_value` / `previous_share_of_bar_draw`: the share is requeue **bar** draw over total bar draw, so it is genuinely 0–1, while `current_value` stays the full bar+blank draw that `by_job.total_value` reports — the extra fields make the ratio auditable instead of implied. (3) Requeue detection runs off the detail row's `reference LIKE 'Job RQ-%'` rather than re-joining jobs, because references are built as 'Job ' || jobs.job_number — same rule as the Part History RQ chip, still one definition of a movement. (4) The scorecard block's rows were chosen here (§6.3 named the section but not its contents).
**Why:** The opening inventory valuation (Sep 2) made raw material a tracked balance; the weekly pack now needs the flows behind it without hand-run SQL. Inclusive dates match how people pick a week; the prior-period comparison is computed, not remembered. No LLM: the story is arithmetic, and the same numbers feed month-end COGS. Inch-only partial usage deliberately excluded (measured negligible).
**Deploy:** SQL on TEST → this round on TEST → verify against the two hand-produced weeks → SQL on PROD → deploy. The migration's DO block proves the RPCs exist with the right guard and grants, that the detail RPC's 12 output columns equal the registry `columns` (the CSV contract), and that the summary re-adds to the detail over a live 180-day window before it commits.
**Open:** the auth guard is inlined in both RPCs rather than routed through a shared helper — report_part_history's SQL is not in the repo (D-RPT-13 cites Docs/migrations/2026-09-01_report_part_history*.sql, which were never committed), so its guard could not be read and reused; the inlined version implements the documented behaviour (any active SkyNet user, NULL uid passes for the SQL Editor). Worth reconciling into one helper when that SQL is recovered.
**Files:** Docs/migrations/2026-09-12_D-RPT-14_inventory_movement_v2.sql (run by Matt, not CC), src/components/reports/InventoryMovementReport.jsx (new), src/pages/Reports.jsx.

**Amendment (2026-09-12, v2 — 42804 on the detail RPC):** v1 never applied. It failed in the verification block with `42804: Returned type character varying(100) does not match expected type text in column 11` — PL/pgSQL's RETURN QUERY requires an exact type match against RETURNS TABLE, and `profiles.full_name` (column 11, recorded_by) is varchar(100), as are several other source columns. The whole migration rolled back on its own BEGIN/COMMIT, so there was no partial state to unwind. v2 casts all twelve outputs explicitly in the detail RPC's final projection, which also makes the column contract immune to a source column changing type later; the summary RPC inherits the fix because it consumes the detail RPC's typed output, and the registry row and verification block are untouched. v1 was deleted rather than kept beside v2 (unlike the D-RPT-13 pair) because it cannot be run successfully and nothing was ever applied from it.

### D-PRICE-42 — Customers: tick-to-list, bought date range, sortable columns; DFAR off printed lists (2026-09-13)
**What:** Purchase history gains a tick column (row + tick-all-shown, scoped per customer), a **Bought from / to** range (part kept when `last_bought >= from` and `first_bought <= to`), click-to-sort headers (shared `useSortedRows` / `SortableTh` in PricingTypeaheads.jsx: asc → desc → off, blanks last, numeric-aware string compare) and a "shown of total · ticked" caption. **Create Price List** passes `partKeys` = ticked parts, or every part shown when none are ticked; the builder narrows `pricing_customer_sheet` to them (compared as a joined string so parent re-renders do not reload the sheet), sorts with the same hook and saves lines in the on-screen order, so the PDF / XLSX print as arranged. The purchase table now prices every row (40 RPC calls per batch, painted as they land) instead of the top 200 by revenue. `priceListDoc.js` drops the DFAR column from the PDF and XLSX; description takes the width; the flag is still stored on `price_list_lines` and shown in the builder.
**Why:** Matt, 2026-09-13: reps need to choose which parts go on a list and in what order; DFAR is not a customer-facing field; the 200-row cap left Irwin's $0 kit-component lines permanently unpriced ("…") and was mistaken for missing prices.
**Deviation from the round brief:** the sort hook lives in src/components/pricing/hooks.js (react-refresh/only-export-components, per D-RMF-04), an empty partKeys array yields an empty sheet, and the DFAR sentence was removed from the printed terms as well as the column. On the first: the brief put `useSortedRows` in PricingTypeaheads.jsx, which exports components, so the rule broke the round's own "0 problems" gate; the file now exports `SortableTh` only and the hook moved out, matching kitregistry/hooks.js and rmforecast/usePartDimensionEditor.js. Repo-wide lint unchanged at 35 errors / 38 warnings; the pricing module, hooks.js included, is at 0.
**Verified:** `useSortedRows` driven directly (17 assertions) — numeric asc/desc, numeric-aware part collation (SK1 < SK2 < SK10, not lexicographic), blanks last in **both** directions for null / 0 / empty string, pass-through on no-sort / null rows / unknown key, no mutation of the source array, the full asc → desc → off cycle plus switching columns, and that the sorted view is what `save` sends. Not verified by execution: the range predicate, the tick/pre-fill wiring and the PDF / XLSX column drop — those are read-through and Matt's TEST pass.
**Not done:** the range is applied client-side on first/last dates, not per SO line; the same picks/range are not yet offered in Quote Builder.
**Files:** src/components/pricing/hooks.js (new), PricingTypeaheads.jsx, PriceCustomers.jsx, PriceListBuilder.jsx, src/lib/priceListDoc.js. No SQL.

### D-PRICE-43 — Instruction sheets excluded from lists (2026-09-13)
**What:** The 14 Fishbowl instruction-sheet products (SK*.DOC, B35-INSTRUCTIONS, Instructions-ZLoc, RV Instructions, PA30C2.DOC) were added to `pricing_excluded_products` with reason "instruction sheet — free document (D-PRICE-43)" (`Docs/migrations/2026-09-13_instructions_exclusion_and_irwin_check.sql`). No code: the table is honoured by `v_customer_purchases`, `pricing_customer_sheet`, both statistics RPCs and the Fishbowl-product typeahead (D-PRICE-28); verified on Irwin — 0 instruction rows in the purchase view and the builder sheet. 1,415 history lines carried these products.
**Why:** Matt, 2026-09-13 — instructions are free documents shipped with kits and must not appear on a customer's price list. Diagnostic on the same day: 15,797 of Irwin's 21,219 shipped lines are $0 kit-component lines (Fishbowl prices the kit header, type 80, which the poller skips by design), which — together with the 200-row pricing cap fixed in D-PRICE-42 — is why parts appeared "without prices".
**Files:** Docs/migrations/2026-09-13_instructions_exclusion_PROD.sql (the block-by-block PROD run, applied by Matt). The TEST run was made from 2026-09-13_instructions_exclusion_and_irwin_check.sql, which is not in the repo.

### D-PRICE-44 — Kit header lines imported into purchase history (step 1) (2026-09-14)
**What:** The bridge's SO-history poller (`q.soHistory` in tools/fishbowl-bridge/src/queries.mjs) now includes Fishbowl line type 80 (Kit) alongside 10 and 12; `fb_so_history_lines.line_type_id` already carried the type. Matt reset `fb_sync_state.history_cursor` and ran `npm run backfill:pricing` to re-read history from 2023-11-27 (the D-PRICE-28 pattern). No schema change. `PRODUCT_LINE_TYPES` (D-FB-08, open-SO dispositioning) is unchanged.
...
### D-PRICE-46 — Kits priced as component sums; hardware at 2× cost; Trim / Fuel Tank retired (2026-09-14)
**What:** Rule (Matt): a kit's list price is the sum of its BOM components at the current book's Each; hardware components are valued at 2 × latest received purchase cost; family ratios 1.0; no minimum-increase floor. `kit_skus.family` added (Cowling / Option / RV / Lancair / Trim / Fuel Tank, seeded from Skybolt_Kit_Pricing_Analysis_v4_072826.xlsx, 405 kits); the 87 Trim and Fuel Tank kits set inactive (discontinued). Rev 82 gained four sections "Skybolt Kits — <family>" with 318 `component_sum` items (`price_items.kit_sku_id` → registry), 2,937 `price_kit_components` rows rebuilt from `kit_bom_lines`, and a "Kit Hardware (cost-based)" resale-kind section (never uplifted) seeded at 2 × last PO cost (17) or Fishbowl sale price marked REVIEW (23); 10 pre-existing unpriced hardware items priced at 2 × cost; instruction sheets (pricing_excluded_products) removed from kit component lists as zero-value documents (61 rows). Result on TEST and PROD: 289 of 318 kits price by rule; 29 blocked by 13 components with no cost on file (MS21059L3 alone blocks 25 RV/Lancair kits). Engine check: Rev 82 Σ = 1.15 × the July workbook Σ to within rounding; kits at ~1.6× Fishbowl list are the 116 never-sold kits July left unpriced (kept rule-priced; Fishbowl inactivation on the backlog). 20 kits land below today's Fishbowl list once the July 10% floor is gone (April's BOM review list; seven have sales this year).
**Why:** Fishbowl kit lists were set on 2026-07-28 to Σ components at Rev 81 (workbook), but only for kits with sales, by hand, with a floor — and the price book had no kits at all. Discovery 2026-09-13/14 (D-PRICE-44) showed kit sales concentrated (Irwin 1,345 header lines / $684K) and Fishbowl kit list prices not maintained for zero-sales kits. One systematic rule replaces the annual spreadsheet and follows every book revision automatically.
**Editor lessons (recorded once, for good):** the Supabase SQL Editor's SELECT…INTO check also fires on *comments*; creating temporary tables inside a DO body triggers the RLS injection and splits the `$$`; DO blocks beyond a screen go through psql (`$env:TEST_DB_URL` / `$env:PROD_DB_URL`, `PGCLIENTENCODING=UTF8`), never the Editor.
**Not done:** Irwin's kit specials — measured against the engine's Tier 3 kit price after Oct 1 rather than assumed; kits without a family (72 registry SKUs, incl. King Air and firewall-only kits) await April; the 13 zero-cost components (D-PRICE-47 fills them from PO history).
**Files:** Docs/migrations/2026-09-14_D-PRICE-46_kits_into_book.sql, …_part2_unblock_and_reports.sql. No code.

### D-PRICE-47 — Latest purchase cost mirrored from Fishbowl; hardware Each = cost × 2 (2026-09-15)
**What:** New mirror `fb_part_costs` (one row per part: latest received PO line — cost, date, PO, vendor, qty — plus Fishbowl std cost), written nightly by the bridge's new part-costs poller via `fb_upsert_part_costs(jsonb)` and on `npm run backfill:pricing`. `price_items.cost_plus` marks cost-derived items (the Kit Hardware section). `pricing_refresh_hardware_costs(p_book, p_markup default 1.0)` — draft only, edit-role gated — sets Each = round(cost × (1 + markup), 3) for cost_plus items and auto-adds any kit component that has a cost on file but no book row, so kit sums unblock as costs arrive. Published books never move; a new draft or the Refresh costs action picks up current costs. `v_hardware_cost_drift` lists active-book hardware more than 10% away from 2 × today's cost.
**Why:** Matt, 2026-09-14: hardware inside kits should be priced from what Skybolt pays, at 100% markup (today's hardware sale prices average 2.07 × PO cost, so no kit steps back). Fishbowl sale prices for kit hardware were unmaintained ($0 on 40 SKUs); PO history is the only reliable source and the bridge already reads Fishbowl's MySQL nightly.
**Bridge notes (CC, 2026-09-14):** the poller has no clock of its own — `fb_sync_state` carries no `last_part_costs_at` column — so it runs immediately after each products poll rather than on a `nightlyDue()` of its own, which with a null clock would fire on every 20 s cycle. `PART_COSTS_ENABLED` (default true) skips it. Two Fishbowl assumptions are unverified and marked TODO in `q.partCosts`: `po.statusId = 70` for Void (the bridge has no PO-status constants; `FB_SO_STATUS` is the sales-order list) and the existence of `part.defaultProductId`. Rows are de-duplicated on the RPC's own key (`upper(coalesce(product_num, part_num))` minus whitespace) before sending, because the whole batch goes through one `ON CONFLICT` statement and Postgres errors on a key repeated within it. `fb_upsert_part_costs` is the only `fb_upsert_*` RPC with no `_pricing_gate`/`_fb_gate` role check while granted EXECUTE to `authenticated` — flagged for Matt, not changed (no SQL by CC).
**Files:** tools/fishbowl-bridge/src/queries.mjs, src/partCosts.mjs (new), src/mapper.mjs, src/skynet.mjs, src/index.mjs, config.mjs, .env.example, README, package.json (1.4.0); Docs/migrations/2026-09-15_D-PRICE-47_part_costs.sql (applied by Matt). UI ("Refresh costs" on a draft, drift badge, Kit Registry price panel) is D-PRICE-48.
**v3 (2026-09-14):** `pricing_refresh_hardware_costs` also adopts kit components that exist in the book unpriced and have a cost on file (sets `cost_plus`, prices at cost × (1 + markup)); found on MS21059L3. File: Docs/migrations/2026-09-15_D-PRICE-47_v3_refresh_adopts_unpriced.sql.
**Result (2026-09-14, PROD):** bridge 1.4.0 first run: 2,116 parts read, 2,109 costs written (7 duplicates collapsed by the bridge's key de-dupe). Refresh on Rev 82: 29 hardware items repriced at 2 × cost, MS21059L3 adopted from an unpriced Resale row ($1.20 → $2.40), 22 seeded items still without PO history. Kit coverage 314 / 318 (RV 37/37, Lancair 6/7, Cowling 163/169, Option 95/105); the 17 open kits hang on 11 components with no cost or no book row (April's list) and one BOM placeholder (VANS RV KIT on LANCAIR-C1P-F); none sold in the last 12 months.

### D-PRICE-48 — Kits in the portal: Fishbowl CSV carries sets and kits; Refresh costs; cost drift; Kit Registry price panel (2026-09-15)
**What:** (A) `productsCsv` now emits `component_sum` items at their resolved Each (`columnPrice` over `_components`), skipping and counting unresolved sums; the export flash reports priced / sets-kits / skipped. (B) Draft toolbar gains **Refresh costs** → `pricing_refresh_hardware_costs(book, 1.0)`; sections containing `cost_plus` rows are tagged cost-based and lose Set section. (C) Active book row shows an amber drift pill from `v_hardware_cost_drift` (>10% from 2 × today's cost). (D) Kit Registry drawer gains a read-only Pricing card: book price today and next-book price via `pricing_get_price`, and the BOM priced line by line with unpriced components marked — one RPC call per component, no bulk load.
**Why:** Finding during D-PRICE-46: the Products CSV exported only `priced` rows, so sets never reached Fishbowl (their Fishbowl list has sat ~25% under book since Rev 81) and kits would not have on Oct 1. Matt's rule (D-PRICE-46/47) needs the refresh action and the drift signal to be usable without SQL, and the Kit Registry is where kit prices are looked up — it reads the book rather than storing a price (single source of truth).
**The $0.00 trap (CC, 2026-09-14):** `loadBookItems` returns bare `price_items` rows with no `_components` — only `loadSectionItems` attaches them — and `columnPrice` sums an empty component list to **0, not null**. Exporting straight from a whole-book load would therefore have written `$0.00` into Fishbowl for all 318 kits and 13 sets. Fixed on both sides: `loadKitComponentsForItems(itemIds)` loads the book's components alongside its items in `PriceBooks.loadBook`, and `productsCsv` treats a `component_sum` with no components as unresolved rather than free. The engine itself is untouched.
**Files:** src/lib/pricing.js, src/components/pricing/PriceBooks.jsx, src/components/kitregistry/KitDrawer.jsx, src/components/kitregistry/KitPricePanel.jsx (new). No SQL.

### D-PRICE-48 addendum — computed sums shown in the Price Books editor and Diff (2026-09-15)
**What:** `component_sum` rows no longer render a bare Σ placeholder. The grid shows the resolved Each (`Σ $3,380.44`, title "Sum of N components"), or `Σ —` in rose titled with the components that block it ("1 of 18 components unpriced: MS21059L3"). The section toolbar counts resolved sums with the priced rows and adds `· K unresolved` in rose; the "vs in-effect book" average still ignores sums, which carry no `list_price` of their own. `diffBooks(base, new, { baseMeta, baseBook, newMeta, newBook })` now compares the resolved Each on both sides, each resolved **within its own book**, so an added kit shows an Each, a set whose components moved shows before / after / % like any priced row, and an unresolved sum shows `Σ —` and is excluded from the Δ. The Diff tab therefore loads the other book's components and meta as well as its items.
**Why:** The editor could show 318 kits with no price at all on screen, so there was no way to see what a kit costs — or which component was blocking it — without running SQL. The diff was worse than blank: sets compare on `list_price`, which is NULL for every one of them, so a kit whose components had all moved read as unchanged.
**Note on the resolve map:** the sum of a set is resolved through a `part_key → item` map memoised once per item load, not rebuilt per row — with 4,500 items and 331 sums a per-row map is quadratic. Components come from `loadKitComponentsForItems` (D-PRICE-48); a sum with no components stays `null`, never 0.
**Verified on TEST (Rev 82):** AC500-C1 `Σ $3,380.44` over 10 components, matching D-PRICE-46 Block 4; RV1014J-C1P `Σ —` naming MS21059L3, 1 of 18; Cowling `169 items · 163 priced · 6 unresolved`.
**Files:** src/lib/pricing.js, src/components/pricing/PriceBooks.jsx. No SQL.

**Addendum 2 (2026-09-15):** A fourth tab, **Kits**, sits between Sections & items and Rules & ladders. Sections are partitioned by **content, not name**: a section belongs to Kits when it holds at least one `component_sum` item, plus the section matching `/kit hardware/i` (which holds cost rows, not sums). Everything else stays in Sections & items, so the catalog no longer scrolls past 300 kit rows and the Common Sets move with the kits. `Resale Items` stays on the catalog side — it carries 10 `cost_plus` rows but no sums, which is exactly why the rule keys on sums rather than on `cost_plus`. Both tabs render the same section list, toolbar, item grid and row renderer — one code path under `view === 'items' || view === 'kits'`, not a second copy — and each remembers its own selected section. Kits-tab section list is ordered Common Sets · Cowling · Option · RV · Lancair · Kit Hardware with `items · resolved · unresolved` counts inline; Set section is hidden there (a sum is not repriced by percentage) in favour of the note "Kit price = sum of components at book price · hardware at 2× cost". Every `component_sum` row carrying a `kit_sku_id` gets a ↗ link to `/kits?sku=<part number>`, a react-router `Link` so a plain click is a client-side nav and Ctrl/⌘-click still opens a tab; `KitKiosk` lands on the Search tab when `?sku=` is present and `KitSearch` opens the drawer on that SKU via the new `skuIdByPartNumber()`, which is deliberately not filtered on `is_active` so the 87 retired Trim / Fuel Tank kits still open. The SET badge reads **KIT** when the row has a registry row and stays **SET** otherwise. Refresh costs is now on the Kit Hardware section toolbar **and** still on the draft toolbar at the top — one shared handler, two buttons, kept in both because the top one reaches a reprice without first finding the right section. Verified on TEST Rev 82: every one of the five sum-bearing sections moves to Kits and none is left behind; AC500-C1 and SK2600FW-SET1 both resolve, so **SK2600FW-SET1 does get a link** (it has a registry row, family NULL); the three Common Sets with no registry row — SK2600RS-SET2, SK2600SW-SET2, ZG2600W-SET1 — correctly show no arrow. **Files:** src/components/pricing/PriceBooks.jsx, src/lib/kitRegistry.js, src/components/kitregistry/KitSearch.jsx, src/pages/KitKiosk.jsx.

### D-PRICE-49 — SkyNet publishes kit prices to the skybolt-kits site (2026-09-15)
**What:** `pricing_kit_prices(p_as_of)` returns each registry kit's resolved Σ, book, unpriced components and BOM from the book in effect on a date. Edge Function `sync-kits` (SkyNet project) reads it and writes the skybolt-kits project (`xqemckrgbirjkicpjfpn`): `kit_pricing` gets a new current row per changed kit (`source = 'SkyNet <book>'`, old row `is_current = false`), unresolved kits keep their price with `source` marked STALE (the site's existing badge), discontinued registry kits are set `status = 'discontinued'` with no current price, `kits.family` follows the registry, and `kit_components` is rewritten from the book's BOM. Matching by exact then whitespace-stripped kit number; unmatched reported both ways; the function never creates or deletes `kits` rows. Dry run is the default; every run is logged in `kits_sync_runs`. Triggers: the portal after Schedule / publish, and the bridge nightly after the products cycle. The site's code is unchanged.
**Why:** Matt, 2026-09-13: "when prices change, we need to immediately update our kits." The site's `kit_pricing` (482 current rows) was the July workbook plus hand edits — a third copy of kit truth beside the registry BOM and the book. With kits priced by rule (D-PRICE-46/47) the book is the single source; the site becomes a mirror. Note: today the book in effect (Rev 81) has no kits, so the first real push is the morning of 2026-10-01 after Rev 82 takes effect.
**Third matching pass (CC):** whitespace stripping alone cannot bridge `4P3-T26` ↔ `SK4P3-T26`, so a third pass drops a leading `SK` from both sides. It runs only for kits still unmatched after the exact and normalised passes, and only when it resolves to exactly one free candidate, so it can never re-point a kit that already matched.
**Precedence (CC):** an inactive registry kit is discontinued even when it is still in the book — a retired kit must never take a new price. The brief listed the in-book branches first; the order here is `!is_active` → `!in_book` → unresolved → priced.
**Safety:** planning is a pure module (`plan.ts`) with no I/O, unit-tested from Node over a fixture covering exact / SK-prefix / site-only / registry-only / changed / unchanged / unresolved / discontinued (31 assertions). Statically verified: no `insert` into `kits`, no `delete` on `kits` or `kit_pricing` — a superseded price keeps its row. `kit_components` is the one table replaced, per kit, because it is derived data.
**Not done:** `dealer_price` / `volume_price` on the site (the engine has Tier 3 and 500+ for kits — a later round); the 6 site-only kits and the spelling variants are reported, not fixed.
**Blocked on Matt:** `pricing_kit_prices` does not yet exist on TEST or PROD, and `kits_sync_runs` exists on PROD only — neither migration is in the repo. The function is written to the documented contract and cannot run until both are applied to both databases.
**Files:** supabase/functions/sync-kits/index.ts, plan.ts, plan.test.mjs, src/lib/pricing.js, src/components/pricing/PriceBooks.jsx, tools/fishbowl-bridge (src/skynet.mjs, src/index.mjs, config.mjs 1.5.0, .env.example, README); Docs/migrations/2026-09-15_D-PRICE-49_pricing_kit_prices.sql, …_kits_sync_runs.sql (applied by Matt — not yet in the tree).

### D-JOBMERGE-20 — Lot-change split measures remainder against the run target (2026-09-15)
**What:** split_job_lot_change (same signature, CREATE OR REPLACE) computes v_run_target = jobs.quantity + Σ active job_merge_allocations.requested_qty and takes v_remainder = v_run_target − p_good_pieces, mirroring getRunTarget(). Merged members are refused (the lot change is declared on the host). Job A finalizes at the made count and keeps its merge allocation rows; Job B is a plain remainder job on the host work order, so D-SHORT-07 orders-vs-stock already treats it as owing only the balance and no allocation rows move. When the host carried members, Job A's paperwork_changed_at/reason are stamped (D-JOBMERGE-03 derived-staleness pattern). job_splits.original_qty_before now records the run target so the row reads target → made + remainder. The error message names the run target. Kiosk guard text aligned.
**Why:** J-000229 (SK27CP8, own qty 2,700 + J-000255 3,000 merged = 5,700) had 4,539 sent across seven batches when the machinist declared lot 2700-series finished. The kiosk (moved to run target in D-JOBMERGE-04) showed a 1,161-pc continuation; the RPC still read jobs.quantity alone, computed 2700 − 4539 < 0 and raised "No remainder left", stopping the machine. D-JOBMERGE-04 updated every machinist-facing quantity in the client and missed the one server-side quantity the kiosk hands off to.
**Known gap:** if a host's made count is below its members' total ask, the members' balance is not carried onto Job B; it surfaces through _reconcile_merge_shortfall at allocation time (D-JOBMERGE-14) for compliance/scheduler to re-merge onto Job B if desired. Separate decision if this occurs in practice.
**Deploy:** Migration applied TEST then PROD 2026-09-15; J-000229 split retried from the kiosk after PROD.
**Files:** Docs/migrations/2026-09-15_split_job_lot_change_run_target.sql, src/pages/Kiosk.jsx, Docs/Decisions.md.

### D-SCHED-25 — Machine change on an approved job is a traveler event, not a compliance reset (2026-09-15)
**What:** reschedule_with_cascade (same signature, CREATE OR REPLACE) no longer reverts an assigned job to pending_compliance or resets job_documents when the target machine differs; p_revert_compliance is retained for signature stability but ignored, and a p_new_status of pending_compliance on an assigned job is coerced back to assigned. The RPC instead stamps paperwork_changed_at/reason ('Machine changed: A → B') — the D-JOBMERGE-03 derived-staleness pattern — calls _notify_compliance('machine_changed', …) and writes an audit_logs 'machine_changed' row. ScheduleJobModal drops the confirm dialog and the "Reschedule & re-review" label; the red panel becomes a blue informational note. applySchedule always passes revertCompliance=false. Compliance Review gains a "Traveler Outdated (n)" worklist (fetchStalePaperworkJobs in jobMerge.js, client-filtered by isPaperworkStale over assigned/in_setup/in_progress/manufacturing_complete) with Print traveler and Acknowledge (ack_job_paperwork); it covers every staleness cause — machine change, merge, unmerge, lot-split host (D-JOBMERGE-20) — and self-clears by timestamp. Kiosk unchanged: the amber stale-traveler banner already renders and start was never gated on it.
**Why:** The S9 flip treated a machine change as if the job were new — compliance status reset, every document back to pending, Roger re-approving the same drawing and production log. Field use (SK27CP3 J-000196 moved to Ganesh 1 while down) showed the documents are part-specific, not machine-specific; the only artifact that changes is the machine on the traveler, which is generated from live data and swaps on reprint. The cost of the reset was a stalled machine and duplicate review work; the compliance need is awareness and a reprint, which the stale-traveler mechanism already provides.
**Deliberately not done:** no change to pending_compliance reschedules (still silent) or to in_setup/in_progress jobs (not reachable from the modal). Server-side role hardening on reschedule_with_cascade remains the D-SCHED-11 RLS-round candidate.
**Files:** Docs/migrations/2026-09-15_machine_change_no_revert.sql, src/components/ScheduleJobModal.jsx, src/lib/scheduling.js, src/lib/jobMerge.js, src/components/ComplianceReview.jsx, Docs/Decisions.md.

### D-SCHED-26 — Traveler Outdated worklist labels the cause (2026-09-15)
**What:** classifyPaperworkChange(reason) in jobMerge.js maps the fixed RPC prefixes of paperwork_changed_reason to a typed chip — Machine change (blue), Merge (amber), Unmerge (purple), Lot split (emerald), Paperwork (grey fallback). Each Traveler Outdated row leads with the chip; the header shows per-type counts. Classification is prefix-only, on strings the RPCs write, never on free text.
**Why:** TEST verification of D-SCHED-25 surfaced eight rows, seven of them merges from Aug 20–Sep 9 that had never been reprinted or acknowledged — correct by the D-JOBMERGE-04 staleness rule, but indistinguishable from the new machine-change rows at a glance. The compliance officer needs to tell "reprint because the run grew" from "reprint because the machine moved" without reading each line. No data change: nothing was newly flagged; the worklist exposed what the Kiosk banner and Mainframe badge already reported.
**Files:** src/lib/jobMerge.js, src/components/ComplianceReview.jsx, Docs/Decisions.md.

### D-SCHED-26a — classifyPaperworkChange prefixes verified against live data (2026-09-15)
**What:** Ordered prefix checks: "Machine changed:" → Machine change; "Merged into" → new Merged member chip (muted amber); "Unmerge" → Unmerge; "Merge:" → Merge; "Lot-change split:" → Lot split; else Paperwork. The colon-anchored host/lot patterns and the "Merged into" test placed ahead of "Merge:" stop the member phrasing from being labeled as a host merge.
**Why:** D-SCHED-26's prefixes were specified from memory because merge_job_into_host and unmerge_job have no source in the repo (JobMerge_R3_TEST_Migration.sql was never tracked; the schema dump has no function bodies). A distinct-value query of jobs.paperwork_changed_reason on TEST confirmed the host and member strings and exposed the overlap. Unmerge remains unconfirmed — no unmerge has ever been performed — and is the first thing to re-check when one is.
**Follow-up:** recover merge_job_into_host / unmerge_job / _job_merge_gate bodies via pg_get_functiondef and commit them under Docs/migrations/ so RPC source is in the repo (same gap as D-PRICE-47/49).
**Files:** src/lib/jobMerge.js, Docs/Decisions.md.

### D-JOBMERGE-21 — Job-merge RPC source recovered into the repo (2026-09-15)
**What:** Docs/migrations/2026-09-15_recover_job_merge_rpcs.sql holds the verbatim PROD bodies of _job_merge_gate, merge_job_into_host and unmerge_job (pg_get_functiondef, 2026-09-15). CREATE OR REPLACE with identical bodies — a no-op against a current database; grants were not captured and are not restated. The file documents the four paperwork_changed_reason prefixes these RPCs write. classifyPaperworkChange gains the matching unmerge split: "Unmerged from" → Unmerged member (muted purple) tested before "Unmerge:" → Unmerge, closing the D-SCHED-26a open item.
**Why:** JobMerge_R3_TEST_Migration.sql (D-JOBMERGE-03) was applied but never committed, and Docs/Supabase SQL Database.txt carries no function bodies, so the only source of these RPCs was the live database. D-SCHED-26 had to guess a prefix as a result. Every deployed function should have a file in Docs/migrations/; this closes the known gap for the merge set. Remaining candidates for the same treatment: any RPC referenced in Decisions.md without a matching Docs/migrations/ file — list them in the next close-out round rather than here.
**Files:** Docs/migrations/2026-09-15_recover_job_merge_rpcs.sql, src/lib/jobMerge.js, Docs/Decisions.md.

## Costing Model — SK2003-10C1 and SK4002-(x)SFW rounds (2026-09-16)

Costing decisions D-COST-01 … D-COST-33 live in `Docs/SkyNet_Costing_Specification` (v0.1 – v0.5) and the workbook README. From this round forward the D-COST series is also logged here so one file carries every decision. Numbering continues from D-COST-33; the collision check was run against the spec and this file (only a D-COST-31 cross-reference existed here).

### D-COST-34 — Highest eligible machine is the costing basis when a part can run on several (2026-09-11)
**What:** When a part is eligible for more than one machine (e.g. "any Mazak or the Ganesh"), the headline cost uses the machine with the HIGHEST burden $/hr among the eligible set; every other eligible machine is shown as a sensitivity row, not as the headline. Burden stays on the lights-out 4,500-hr basis (D-COST-27) for 24-hr bar-fed parts and the 1-shift basis for attended setup.
**Why:** Machine assignment is the largest single swing in a machined-part cost and is an allocation choice (whose labor a pool carries), not a manufacturing fact — SK2003-10C1 moves 4× between the Ganesh ($0.13) and Mazak 1/2 ($0.52) with nothing about the part changing. Costing high is the conservative internal standard; the sensitivity block keeps the cheaper truth visible. First use: SK2003-10C1 on Mazak 1/2 ($11.72/hr lights-out) vs Ganesh 1 ($1.63/hr).
**Files:** SkyNet_Costing_Model_v0_9_9.xlsx (tab SK2003-10C1 Product §5), Skybolt_Part_Cost_Breakdown_SK2003-10C1_DRAFT1.pptx, Docs/Decisions.md.

### D-COST-35 — Internal components with no sell price are costed to factory cost with volume from the parent product (2026-09-11)
**What:** A component that is never sold on its own (e.g. SK2003-10C1, shipped to the rubber vendor and returned inside the SK2003-X Platemount) is costed to factory cost INCLUDING the 20% manufacturing-support overhead, with no margin, quote or price-to-target section. Annual volume is taken from Fishbowl shipments of the parent product(s), one component per parent unless the BOM says otherwise.
**Why:** The Platemount carries the price; the component has none, so a margin section would be invented. Overhead is included because the component is the whole product from Skybolt's side (there is no later Skybolt roll-up to carry it — D-COST-21 applies once, here). First use: SK2003-10C1 = $0.518/pc at 42,567/yr (2025 SK2003-14A/-24A/-42A shipments).
**Files:** SkyNet_Costing_Model_v0_9_9.xlsx (tab SK2003-10C1 Product §1/§4), Docs/Decisions.md.

### D-COST-36 — Comparable-part cycle time is admissible when the part has no timestamped run (2026-09-16)
**What:** Where a part has no SkyNet job with production_start → actual_end and no send-waypoint recovery is possible (D-COST-31), the elapsed cycle of the closest MEASURED comparable — same feature set, same bar size and material, same machine class — is admissible as the costing rate, flagged COMPARABLE in the workbook and deck. It is superseded by the part's own harvest the first time one exists (D-COST-26 precedence: measured → send-derived → proxy → comparable → hand estimate).
**Why:** The SK4SFW(x) stud has one legacy manual batch (J-000007) with no timestamps, and the SK4C8W1 / SK4C10W1 wing studs (J-000169, J-000241) are still in setup; a hand estimate would have been the only alternative. The 2000-series folding wing stud SK26SFW3/5 — same slotted head + cross-pin geometry, 3/8" 303, Mazak 1 — has three measured jobs (9,706 pcs, 844 hrs, 5.22 min/pc). First use: SK4SFW(x) ← SK26SFW3/5. Re-harvest when J-000169 completes.
**Files:** SkyNet_Costing_Model_v0_9_10.xlsx (tab SK4002-SFW Product §3/§8), Skybolt_Part_Cost_Breakdown_SK4002-SFW_DRAFT1.pptx, Docs/Decisions.md.

### D-COST-37 — DFARS / US-sourced components: domestic quote + tooling pass-through, no duty, post-processing in-house inside conversion, assembly rides the standard list (2026-09-16)
**What:** A purchased component re-sourced to a US vendor to meet DFARS is costed at the domestic quote for the actual release quantity, plus any one-time engineering / die charge as a pass-through (D-COST-32) amortized over that release (conservative; longer amortization shown as sensitivity), with no import-duty layer. Any finishing the domestic vendor does not perform (deburr, polish, stamp) and Skybolt does in-house is a direct-labor line INSIDE the conversion / 20% overhead base. The assembly keeps its standard Pricing Portal price — no separate DFARS part number or uplift — so the sourcing premium is absorbed in margin and reported as its own line (DFARS cost vs. import-comparison cost).
**Why:** The customer requirement is DFARS material, not a new product; Matt's call is that it rides the standard list. The China comparison stays on the deck to size the premium, not as an option. First use: SK4FW2S wing via Wrico (quote 105782-26) — $0.517 @2,500 + $3,600 die ÷ 2,500 + $0.037 in-house tumble/polish/stamp = $1.99/pc vs Maxtrust $0.21 FOB + 35% duty = $0.28 landed; premium $1.72/unit, assembly $4.97 vs $3.25.
**Files:** SkyNet_Costing_Model_v0_9_10.xlsx (tab SK4002-SFW Product §4), Skybolt_Part_Cost_Breakdown_SK4002-SFW_DRAFT1.pptx, Docs/Decisions.md.

### D-COST-38 — Dash families are costed at the average dash unless a single dash is asked for (2026-09-16)
**What:** For a part family that differs only by length (grip dash), the headline cost uses the arithmetic mean of the length-driven geometry across the dashes in the family (stud L over -2 … -20; cup Dim A over the standard-cup dashes), machine cycle blended across the measured runs of the family, and the headline price is the simple mean of the family's Pricing Portal list prices (Tier 3 = list × the rule multiplier). Single-dash costs are derived from the same tab by overriding the length cells.
**Why:** Length moves material by a few cents (SK4SFW -2 → -20 is ~$0.04 of bar) while list price moves 3× ($11.58 → $33.63); an average-dash cost against an average price is the honest family view, and per-dash margin is read off the price table, not re-costed. Per Matt, 2026-09-16. First use: SK4002-(x)SFW — stud L 1.160", cup A 0.397", avg list $25.53 / Tier 3 $16.08.
**Files:** SkyNet_Costing_Model_v0_9_10.xlsx (tab SK4002-SFW Product §3/§6/§7), Docs/Decisions.md.

### D-PRICE-50 — Engine: kit sums all-or-nothing and set-based; customer sheet prices the book once (2026-09-16)
**What:** `pricing_item_prices` computes kit sums with one grouped join (was a correlated subquery per sum-row-per-column): Rev 82 9.9 s → 0.86 s. Both it and `pricing_get_price` now return NO price for a kit with any unpriced component (was a partial sum: B95-C1 $515.86 with 1 of 7 missing); `pricing_get_price` reports "N of M component(s) without price". `pricing_customer_sheet` calls the book pricer once and pivots q100/q300/q500 (was three calls per row); Lanzen's Oct 1 sheet 1.2 s (was a statement timeout). Same signatures.
**Why:** A sales rep's Oct 1 price list timed out (D-PRICE-46 added 331 sums to Rev 82). Diagnosis found the partial-sum flaw — the client and CSV already treated such kits as unpriced; the engine (Quote Builder, price lists, kits site) did not.
**Files:** Docs/migrations/2026-09-16_D-PRICE-50_engine_kit_sums.sql (applied TEST + PROD by Matt).
**Deployment confirmed (CC, 2026-09-16):** `pricing_item_prices` on both TEST and PROD carries the grouped `simple_prices` rewrite and the `COUNT(kc.item_id) = COUNT(cp.unit_price)` all-or-nothing test, and `pricing_get_price` on both carries the "component(s) without price" branch. This entry's claims are as deployed.

### D-PRICE-51 — Column tiers; tier assignment = admin or pricing_manager; quote deviations view (2026-09-16)
**What:** `customer_pricing.tier` accepts q100/q300/q500; `pricing_get_price` prices a column-tier customer at that column regardless of quantity (`basis = 'column'`), or Each if the ladder lacks it — "never worse, never better" (Matt). `pricing_set_customer_tier` gated by `_pricing_tier_roles()` = admin, pricing_manager; April (abraun) given `pricing_manager` in `profiles.roles[]`. `v_quote_deviations` exposes every quote line priced away from the recommendation with kind (manual / special / column / price) and Δ.
**Why:** Sales-team requests 2026-09-16: some customers should live on the 100/300/500 column; only Matt and April assign tiers; Matt and the sales manager want to see manual prices and off-tier quoting.
**Files:** Docs/migrations/2026-09-16_D-PRICE-51_column_tiers_roles_deviations.sql.
**Deployment as at 2026-09-16 (verified by CC, read-only):** applied to **TEST only**. On PROD none of it is present — `customer_pricing_tier_check` is still the five-value list, `_pricing_tier_roles()` and `v_quote_deviations` do not exist, `pricing_get_price` has no column-tier branch, and April's `profiles.roles` is empty. **The D-PRICE-52 UI is written to this contract and the column-tier picker, the Set tier gate and the Deviations tab cannot work on PROD until this migration is run there.** Separately, the migration's `update … where username = 'abraun'` matched no row on TEST (TEST has no such profile), so on TEST only `admin` holds the tier/deviations rights until someone there is given `pricing_manager`.

### D-PRICE-52 — Sales-team round: search finds sections, Export all, column tiers, manual price, Deviations, Kits in the Catalog (2026-09-16)
**What:** (A) Catalog search lists matching sections above parts. (B) Export… builds a book-wide internal XLSX from the client engine with a column picker (all columns default). (C) Tier picker gains 100/300/500 column; Set tier renders only for admin/pricing_manager; badges and Quote Builder recommendation handle column tiers. (D) Quote Builder Manual chip: price + required reason, `basis = 'manual'`, `is_override`, never a special; rose MANUAL tag internally, plain price on the customer PDF. (E) Deviations tab (admin/pricing_manager) over `v_quote_deviations` with filters and a summary strip. (F) Catalog Kits tab: reads the scheduled book with a *Fishbowl today* column while the book in effect has no kits; becomes the current book on Oct 1.
**Why:** Sales-team feedback session 2026-09-16 (Matt). Kits: option (a) chosen — show Oct 1 prices beside today's Fishbowl list rather than seed Rev 81.
**Files:** src/lib/pricing.js, src/lib/pricingView.js (new), src/lib/catalogExport.js (new), src/lib/roles.js, src/components/pricing/PriceCatalog.jsx, PriceCustomers.jsx, QuoteBuilder.jsx, PriceBooks.jsx, PriceDeviations.jsx (new), src/pages/Pricing.jsx; tests src/lib/pricingEngine.test.mjs, pricingView.test.mjs, catalogExport.test.mjs (all new). No SQL.

**Two pure modules, because the round is testable only if they exist.** `pricing.js` imports the Supabase client, which reads `import.meta.env` and cannot be constructed outside Vite, so nothing that imports it runs under Node. The tier vocabulary, the deviation filter/summary, the Catalog section-name match and the kit-section partition therefore live in `pricingView.js` (no I/O), and the export's column derivation and row builder in `catalogExport.js` (SheetJS only, with the pricer injected). `pricing.js` re-exports `TIERS / TIER_LABELS / TIER_COLORS`, so every existing `import … from '../../lib/pricing'` is unchanged. The engine mirror itself cannot move, so `pricingEngine.test.mjs` loads the real `pricing.js` with only its Supabase import textually stubbed — the functions under test are the shipped ones.

**`bookPricer(meta, book, resolve)` — new in pricing.js, and the round's one real correctness find.** `columnPrice()` is the raw multiplier maths; the two guards the RPC applies were until now written inline in the Catalog grid only. Reading the deployed `pricing_item_prices` body showed the column set is built **per item, from that item's own ladder** (+ Each, + Premier only when `has_premier`), and only then are components summed. So a kit on the `q100_q300_q500` ladder has **no Tier 1/2/3 price in the book at all**, even though every component does — confirmed on TEST Rev 82: 334 kit rows at q100/q300/q500, **0** at tier1/2/3 and 0 at premier. A whole-book export applies every column to every row, so without that guard the sheet would have invented Tier 3 and Premier prices for all 331 kits. `bookPricer` applies the ladder and Premier guards before the sum and is used by the grid and the export alike; `columnPrice` is untouched (round constraint). `sumDetail()` returns the same sum with its workings (`{ value, total, missing[] }`) so a blocked kit renders `Σ —` naming the component that blocks it.
**Verified against the deployed engine** on real Rev 82 rows pulled from TEST: SKFO65-40.25 (Each 59.296, 100+ 56.92416, nothing at 300+ — it is on the `q100` ladder), SK2600FW-SET1 (21.130 / 20.2848 / 19.0170 / 17.5379 and no Tier 3), AC500-C1 (Each only — MS51957-45 sits on the `none` ladder, so the quantity columns do not resolve), RV1014J-C1P (nothing anywhere, blocked by MS21059L3). 47 assertions, the mirror matching `pricing_item_prices` to 1e-6 including every null.

**Which book the Catalog's Kits tab reads — the brief's premise was wrong, so the rule is a comparison.** "The book in effect has no kits until Oct 1" is not true of the data: Rev 81 already carries one sum-bearing section, the 16 CLoc 2000 Common Sets, 13 of them linked to the Kit Registry. Neither "holds a `component_sum`" nor "links to the registry" separates Rev 81 from Rev 82. What actually arrives on Oct 1 is the four kit families plus Kit Hardware — **Rev 81 has 1 kit section, Rev 82 has 6** — so the rule implemented is: *the scheduled book wins while it has MORE kit sections than the book in effect; otherwise the book in effect; otherwise an empty state.* It needs no section names, gives Matt the Oct 1 kits today with the Fishbowl column, and self-corrects on Oct 1 when the comparison ties. The Catalog tab keeps every section of the book in effect until the Kits tab is reading that same book — otherwise today's Common Sets would disappear from Rev 81's catalog to make room for a tab showing Rev 82.

**Columns in the export: 14, not the 8 the brief listed.** The picker is built from `price_ladders.columns` as specified, and Rev 81/82 carry twelve distinct ladder columns (q2 q5 q10 q20 q25 q50 q100 q300 q500 tier1 tier2 tier3) beside Each and Premier. All are ticked by default; All / Each-only shortcuts sit under the list. The count line on PROD today therefore reads **4,244 priced parts · 16 sets/kits · 14 columns** (the brief's "327 sets/kits" is Rev 82's number, not the book in effect's).
**Frozen header.** SheetJS community writes a fixed `<sheetViews>` with no pane element and exposes no hook for one, so `freezeTopRow()` rebuilds the finished workbook's zip with the pane patched in — safe because SheetJS stores entries uncompressed, and it returns the bytes untouched if anything is not as expected, so the worst case is an export without the freeze rather than a broken file. Verified by round-tripping through `XLSX.read` and by `unzip -t` on a 4,500-row workbook. The sheet also carries an autofilter on the header and `0.00` on every price cell; it is written STORED, so a full 14-column Rev 81 export is a few MB.

**Deviations.** `loadQuoteDeviations({ from, to, rep, kind })` pushes all four filters to the view, but the tab loads the date range only and narrows rep and kind with the pure `filterDeviations()`, so the rep list stays complete and switching a dropdown costs no round trip. There is no standalone quote view in the portal to open, so a row click opens a **read-only** panel over `loadQuote` (header, lines, the manual reason, a PDF button) rather than reopening the quote into the Quote Builder draft — this screen writes nothing.

**Smaller decisions.** The Manual chip sits beside the column chips and is therefore offered only where the book prices the part; quoting a part the book cannot price is not part of this round. Manual lines carry `note` through `pricing_save_quote` (the RPC already accepted it; the client simply never sent it) and back through Reopen. `override_count` counts `is_override`, which a manual line sets, so it already includes them — confirmed in the RPC body. The kit-section partition and ordering moved from `PriceBooks.jsx` into `pricingView.js` and PriceBooks now imports them, so "same partition rule as the Price Books Kits tab" is the same code rather than a copy.
**Gates, statically checked:** `setCustomerTier` is called from `SetTierPanel` only, which renders only under `canSetPricingTier`; `PriceDeviations` renders only under `canSeePricingDeviations` and its tab is filtered out of the strip otherwise; `upsertException` appears only in `AddExceptionPanel`, never on any manual-price path.
**Gates:** CRLF throughout; pricing module lint 0; repo baseline unchanged at 35 errors / 38 warnings; `npm run build` clean; 115 assertions across the three new test files, plus the 31 of `sync-kits/plan.test.mjs` still passing.
**Addendum (2026-09-16):** customer PDFs print MM/DD/YYYY; FOB Point column and "FOB Origin" removed from the quote (and price list, if present). One formatter `fmtUsDate` in pricing.js, printed pages only — ISO stays in the database, the portal and every sort. Also applied to the quote's `(as of …)` line, which is a printed date too. The price-list XLSX header is unchanged and still carries the ISO effective date. Files: src/lib/pricing.js, quoteDoc.js, priceListDoc.js, pdfDocs.test.mjs (new — renders both documents and reads the text back: three US dates on the quote, two on the price list, no ISO date and no FOB anywhere in either).

### D-WOLOOKUP-QTYEDIT01 — Quantity edits allowed on scheduled jobs until production starts (2026-09-17)
**What:** EditWorkOrderModal's isEditable widens from pending_compliance/ready to also cover assigned/in_setup. Unscheduled jobs keep the direct client write; scheduled jobs call new RPC update_job_quantity (SECURITY DEFINER, _job_merge_gate admin/scheduler, NULL-uid passthrough), which refuses in_progress and later, merged members, and maintenance jobs; floors at 1; stamps paperwork_changed_at/reason 'Quantity changed: A → B (run target X → Y) — <reason>' (D-JOBMERGE-03 pattern) and notifies compliance ('job_quantity_changed') only for assigned/in_setup; audits every change. Window rule: an assigned job's reduction scales estimated_minutes by new/old run target and pulls scheduled_end in (cannot create an overlap); an increase leaves the window and the return payload says to re-plan on the board; in_setup windows are never touched. Traveler Outdated gains a cyan "Qty change" chip. The WOA Order/Stock inputs unlock with the jobs.
**Why:** J-000279 (2,000) was merged into stock host J-000231 (5,000, assigned) on WO-2608-0075, taking the run target to 7,000 when the intent was to fold the demand into the existing 5,000. The stock quantity could not be cut to 3,000 because the modal locked quantities once a job was scheduled. The lock existed to protect the printed traveler and the schedule window; both are now handled — the traveler by the stale stamp compliance already works from, the window by the shrink-only rule. Production start remains the hard line: after pieces exist, quantity changes are lot splits and merges with their own audit trails.
**Files:** Docs/migrations/2026-09-17_update_job_quantity.sql, src/lib/jobMerge.js, src/components/EditWorkOrderModal.jsx, Docs/Decisions.md.

### D-SCHED-27 — Length-family scheduling history, rate ladder, first-run flag (2026-09-22)
**What:** Machining families keyed as the part number with the length dash masked (`parts.length_family_key`, e.g. SK4C#S; `length_dash`, `length_family_source`), derived by `derive_length_family()` — tier 1 from a leading "-N " in the description, tier 2 from the trailing digit run when the description has no dash (SK26CS#, SK27CP#, SK26-S#…); tier 2 trusted only for families with ≥2 members or already present; stored, never overwritten by the rule, manual edits recorded as `manual`. Strike list at seed: SK244-# (thread sizes), SK21077-5-# (insert sizes). Seed: 306 parts / 28 families. `family_key` (D-AISCHED-05, pricing grain) untouched. Views: `v_part_run_rates` (one row per production-done run; calendar rate on the effectiveTimePerUnit basis, steady rate on the D-COST-31 waypoint basis, paper-era rows flagged and never rated), `v_length_family_machine_history`, `v_length_family_material`, `v_part_observed_material`, `v_job_first_run`. `part_machine_stats` repaired to `coalesce(production_start, actual_start)` and the app's pieces basis — it saw 21 of 184 production-done runs because the kiosk never writes `actual_start`; Uncle Bob's HISTORY source now sees kiosk-era runs. Schedule modal: Step 1 gains FAMILY HAS RUN HERE (violet, between proven and other machines), projected finish vs target per card, material affinity per card (same bar / same material / change), standing rules panel, first-run banner; Step 3 gains the evidence block and Target row. Rate ladder (lib/scheduling.js buildRateLadder), calendar basis only: part on machine → family on machine → part elsewhere → family on same model → family anywhere; steady rate is context and a one-click override, never the default. Adjust End Date shows family evidence with Use buttons, display-only. `fetchPartThroughputRuns`/`fetchPartMachineHistory` moved onto `v_part_run_rates` — resolves the D-SCHED-19 drift note (derived rates now enter the duration suggestion; paper-era rows cannot). scheduleSnapshot reads `length_family_key` + `v_length_family_machine_history` mapped to the existing envelope shape. First-run flag (`part_and_family` / `part`) on the kiosk active-job header and the Production Display (row badge + "First runs — production meeting" panel).
**Why:** SK4C4S (J-000277): the modal showed one machine at 502/day while six neighbouring lengths had run on Nexturn 7 at ~1.4× the rate, invisible because `family_key` merged steel and stainless (SK4-S) and `part_machine_stats` keyed on a column the kiosk never writes. SK26CS6 (J-000273) was scheduled at 500/day from an empty Step 3 while the family's run on the same model said 321 calendar / 435–511 steady. Matt, 2026-09-22: lead a scheduler who may know nothing about the parts or machines; calendar rate is the guide, the end date is the judgment; material changes cost scrap-bin swaps and bar pulls, so "prefer to stick with what's running"; first runs must be visible to the scheduler, the machinist, and the production meeting.
**Deferred to D-SCHED-28:** weekend behaviour per machine (advisory only — the Monday-pickup measure is real but noisy at today's sample sizes and confounded by part mix at the machine level; machine monitoring supersedes it), material sufficiency (bars needed vs material_availability), family campaigning in the pool. Reliability badge held (Matt).
**Files:** Docs/migrations/2026-09-22_D-SCHED-27_length_family_history.sql (TEST→PROD, manual), src/lib/scheduling.js, src/lib/scheduleSnapshot.js, src/components/ScheduleJobModal.jsx, src/pages/Schedule.jsx, src/pages/Kiosk.jsx, src/pages/dashboards/ProductionDisplay.jsx, Docs/Decisions.md.

**Also found (CC, TEST verification 2026-09-22):** moving `fetchPartThroughputRuns` onto `v_part_run_rates` silently fixed a second defect nobody had logged. The old fetcher gated only on `time_per_unit > 0` and carried **no status filter at all**, so cancelled jobs fed the duration suggestion — SK4C15P's only "history" was J-000123, a cancelled run, and it was quoting 188/day off it. The view applies the production-done status set, so cancelled and merged runs are now excluded, which is what D-PARTHIST-02 already said should happen everywhere. Measured across TEST's 100 parts that carry a suggestion: 90 unchanged, 9 changed by the intended derived-rate alignment (largest movers SK212-12S 333→150 and SK28C 616→393, both because a big kiosk-era run with production dates but a NULL `time_per_unit` was previously invisible — J-000093 at 5,293 pcs and J-000059 at 12,851 pcs), 1 new (SK26R8S, no suggestion before → 663), 1 withdrawn (SK4C15P, the cancelled-job case above). The direction is consistently downward toward observed reality, which is the point of the alignment. Pre-existing junk rates left untouched and still visible (SK26C8W1 144,000/day; ZG26-1W1 and "Tools for Assembly" 72,000/day) — data-quality candidates, not scheduling logic.

### D-SCHED-27a — Material affinity against the queue neighbour; material on the grid; part hover card (2026-09-23)
**What:** The Step 1 material line compares this job's material with the job it will land behind — the queue tail (`lastJob`, which is the running job when nothing else is queued) — and names it: "Material change after SK26FB4 (last in queue): 41L40 Steel → 303 Stainless Steel (scrap bins + bar pull)". Step 3 recomputes against the actual predecessor once the slot is chosen. Materials resolve by ladder (`resolveJobMaterial`): loaded `job_materials` row → part's observed bar → family's observed bar → master `material_types` (alloy only, no bar size; 316/436 parts). Blank-stud stock reads "Same stock as / Stock change after", never "bar pull"; unknown bar size reads "Same material as … (bar size unknown)". Timeline blocks (medium/large) show the material short code beside the part number. A hover card (250 ms) on blocks and Job Pool cards shows part · job, description, material + source, family + dash + first-run flag, qty/customer/due, and scheduled range; native `title` tooltips removed from blocks. Board/pool selects embed `length_family_key, length_dash, material_type(name, short_code)` (two levels).
**Why:** Matt, 2026-09-23 (PROD, SK27CP19 J-000280): the card compared against the running part, not the tail, and did not say which — unreadable for exactly the scheduler D-SCHED-27 is meant to lead. Queued jobs carry no material row until the kiosk load, so the neighbour's material has to come from the part. Description and material on the grid because "a scheduler who may know nothing about the parts" was the goal.
**Files:** src/lib/scheduling.js, src/components/ScheduleJobModal.jsx, src/pages/Schedule.jsx, Docs/Decisions.md. No migration.

**Notes from the build (CC, 2026-09-23):** (1) A fourth read was added beyond the three the brief specified — `material_types(name, short_code)` once per page load, cached by name. Master data carries a short code but `job_materials` and the observed-material views do not, so without it a loaded or observed material rendered on the grid as a 12-character truncation ("303 Stainles") rather than a code. The tag now reads the short code whatever the source, falling back to the truncation only when no code exists. (2) The two block wrappers lost their native `title`, but the per-icon `title` attributes *inside* `JobBlockContent` (overrun, missed slot, projected, combined run, stale target, complete) were kept — they carry state the card does not, and the brief's card contents do not cover them. Hovering an icon can therefore still raise a native tooltip alongside the card; worth a look if it reads badly. (3) The neighbour resolution matters more than expected: of 21 machine queue tails on TEST, only 2 had a `job_materials` row (both `in_progress`) — every other tail resolved through the part, the family, or master data, so before this round 19 of 21 cards had nothing to compare against at all.
### D-FB-39 — Fishbowl inventory and SkyNet open jobs in Create Work Order (2026-09-23)
**What:** Create WO shows the D-FB-33 snapshot for the product being ordered (full line on "Job to Create" for finished goods/single parts, and beside "Select parts to add jobs" for assemblies) and a compact chip on every BOM row (nested tree and flat fallback), plus a freshness line under Products. One helper, `summarizeFbInventory` (lib/fishbowl.js), now also drives the Order Queue Avail cell, so the two surfaces cannot drift. Product lines use the ship basis (`qty_available`, Main + Warehouse, D-FB-33 unchanged); BOM rows use a use basis (free stock across every location group, because components sit in Material/Manufacturing). States: current; stale (`snapshot_at` more than 10 min before `fb_sync_state.last_inventory_at` — the part left the bridge's scope and its row froze; 113 of 443 rows on 2026-09-23); not synced (a Fishbowl part the bridge doesn't read); not in Fishbowl (no `fb_products` row). Each line also shows the SkyNet quantity in open jobs, because Fishbowl alone misleads: SK-O read 6,426 short while J-000219 had 7,550 in progress. Read-only; nothing automates on it (D-FB-20); `+ Stock` is never prefilled.
**Why:** Matt, 2026-09-23 — see Fishbowl inventory for the selected part and its components when creating a WO.
**Open (bridge, Matt):** component inventory needs the bridge to read `qtyinventorytotals` for SkyNet parts beyond sales-order lines, and to write zero rows for parts with no totals so absence means zero and rows cannot freeze. No frontend change is needed when it lands — the chips light up.
**Files:** src/lib/fishbowl.js, src/components/FbInventoryChip.jsx (new), src/components/CreateWorkOrderModal.jsx, src/components/NestedBomTree.jsx, src/components/orderqueue/SOCard.jsx, Docs/Decisions.md. No migration.

**Notes from the build (CC, 2026-09-23):** (1) `fetchOpenJobQtyByPart` landed in `CreateWorkOrderModal.jsx`, not `lib/jobs.js`: that module imports no supabase client, and it has no constant for this terminal set either — `EXCLUDED_STATUSES` is cancelled/merged only, so `['complete','cancelled','merged','incomplete']` is declared beside the loader. (2) The AvailCell refactor was proved rather than assumed: the pre-refactor computation was copied verbatim into a test and run against 11 real PROD `fb_part_inventory` rows (multi-location-group, on-order, negative and positive availability, frozen rows) × 5 `need` values — **55/55 identical tone and tooltip**. The ship-basis title deliberately keeps the original expressions on the original raw fields, including the truthiness test on `qty_on_order`, so any quirk there is reproduced rather than "fixed". (3) `by_location` is the use basis's source; when a row somehow carries no groups it falls back to the aggregate columns rather than reporting zero free.

### D-CODATE-02c — Job Pool sort: Part # and Target Date (2026-09-23)
**What:** The Job Pool "Sort by" menu is Work Order #, Part #, Target Date, Customer, Priority. Due Date is removed. Target Date sorts on the date the pool card shows — the D-CODATE-02 SkyNet target from `v_wo_dates`, else the WO due date — earliest first, undated last, tie-break WO number then part number. Part # sorts naturally (SK27CP9 before SK27CP19). Priority sort fixed: `|| 2` had turned critical (0) into normal, so critical jobs never sorted first; now `?? 2`.
**Why:** Matt, 2026-09-23. The pool card already leads with the SkyNet target; sorting on the WO due date sorted on a date the card doesn't show.
**Files:** src/pages/Schedule.jsx.

**Note (CC):** `numeric: true` collation compares each digit run as a number, which is what makes SK27CP9 precede SK27CP19 — and, by the same rule, SK26CS6 (26) precede SK2018-A21 (2018) and SK4C4S (4) precede SK27CP19 (27). That is inherent to natural ordering, not a defect, but it is the behaviour to expect when the pool mixes series. Dates compare as `slice(0,10)` strings so DATE and timestamp forms sort together without reintroducing the D-CODATE-02b UTC-midnight shift. Wrapping the priority case in braces also clears a pre-existing `no-case-declarations` error, so Schedule.jsx lints one error cleaner than `main`.

### D-FB-39a — Correction before merge: one no-row state, one availability basis (2026-09-23)
**What:** D-FB-39 as first built labelled a part "not in Fishbowl" when it had no `fb_products` row, and gave BOM rows a separate "free to use" basis summed across all location groups. Both were removed before merge. A part with no inventory row always reads "Not synced from Fishbowl" (`FB —`); every line — product and BOM — uses `qty_available`.
**Why:** A read-only Fishbowl query on 2026-09-23 showed SK4FB13S with 53 on hand in Main but no Fishbowl product, so `fb_products` cannot identify parts that aren't in Fishbowl. The same query put 4,784 of 4,787 inventory-totals rows in Main and none in Material or Manufacturing, so the second basis produced the same number under a different explanation. The D-FB-39 entry stands as the record of what was first built; this entry supersedes its "not in Fishbowl" state and its use basis. Once the bridge reads every SkyNet part (open item under D-FB-39), a part with no row genuinely isn't in Fishbowl, and the no-row text becomes "Not in Fishbowl" — a one-line follow-up.
**Files:** src/lib/fishbowl.js, src/components/CreateWorkOrderModal.jsx, src/components/FbInventoryChip.jsx, the D-FB-39 tests, Docs/Decisions.md.

### D-PROC-01 — CC builds, Matt reviews in Dev, then CC commits (2026-09-23)
**What:** From here on, CC builds a change in the working tree and stops: no commit, merge or push until Matt has reviewed it in Dev (localhost) and says so. CC's report ends with what to check and which branch is checked out. One change is in flight at a time, because uncommitted work would otherwise collide. When Matt approves, CC commits, merges to test and main, and pushes, in one step.
**Exception:** Dev runs against the PROD database, so a change that needs a migration cannot be reviewed there until the migration is on PROD. Those keep the TEST-first route: migration on TEST, review on test-skynet (or Dev pointed at TEST's database), then PROD.
**Why:** Matt, 2026-09-23 — CC committing and merging before Matt could see the change made the branch state confusing (two parallel branches, a Decisions.md append conflict on test).
**Files:** Docs/Decisions.md.

### D-FB-39b — Open jobs in the inventory tooltip; leftover basis argument removed (2026-09-23)
**What:** `fbSummaryFor` now passes each part's SkyNet open jobs to `summarizeFbInventory`, so every inventory tooltip — product lines and BOM rows — lists them (up to three with status, then +N more). Compact BOM chips also show `· {qty} in jobs` when a component already has open SkyNet jobs. SOCard's leftover `basis: 'ship'` argument (ignored since D-FB-39a) and the comment describing it are removed.
**Why:** CC audit, 2026-09-23. The open-jobs option was never passed, so no tooltip showed it. Compact BOM chips have no visible SkyNet line, so a component with an open job — SK4FB13S, J-000292, 150 pcs, pending compliance — showed no sign of it, which invites a duplicate job.
**Files:** src/components/CreateWorkOrderModal.jsx, src/components/FbInventoryChip.jsx, src/components/orderqueue/SOCard.jsx, Docs/Decisions.md (plus a test file if the repo has a runner).

**Note (CC):** the repo does have a runner — the `node src/lib/*.test.mjs` convention from D-PRICE-52 — so the AvailCell equivalence check is now standing as `src/lib/fbInventory.test.mjs`. It loads the shipped `fishbowl.js` with only its Supabase import stubbed (the D-PRICE-52 shim pattern), embeds the pre-D-FB-39 AvailCell computation from `af5900d^` verbatim as the oracle, and compares tone and tooltip across 11 synthetic rows shaped like the PROD ones (multi-group, on-order, negative and positive available, unmapped group, no `by_location`, no snapshot, frozen) × 5 need values — 55 pairs, plus the D-FB-39 state rules and the D-FB-39b tooltip. 149 assertions. It was mutation-checked rather than trusted: moving the tone boundary, dropping the On-order line, collapsing the breakdown separator and swapping the U+2212 minus for an ASCII hyphen each fail it.

### D-FB-40 — Bridge 1.6.0: inventory snapshot for every SkyNet part; absence written as zero (2026-09-23)
**What:** The inventory poller's scope is the union of parts on open sales orders (D-FB-33), every SkyNet `parts.part_number`, and every part already in `fb_part_inventory` — about 1,300 parts, resolved to Fishbowl parts through `part.num`. `q.inventory` now starts from `part` and left-joins `qtyinventorytotals`, so a Fishbowl part with no stock record is written as a zero row (`by_location = {}`) instead of being skipped; a number Fishbowl does not know gets no row, so "no row" means "not a Fishbowl part". Every in-scope row is sent every cycle, which keeps the D-FB-39 stale test valid. New `INVENTORY_DRY_RUN` switch (only the literal `true` enables it) builds and logs the payload without writing. Aggregation and zero-fill are a pure function. `fb_upsert_inventory`, `AVAILABLE_LOCATION_GROUPS`, batching (300 in / 500 out) and cadence (5 min) are unchanged.
**Why:** Matt, 2026-09-23: all inventory should flow into SkyNet. Under D-FB-33, 3 of 402 manufactured and 8 of 61 purchased SkyNet parts had a row, and 113 of 443 rows had frozen at their last values after leaving the open-SO set. Fishbowl holds the missing parts (SK4C13C 1,750, SK4FB13S 53, SK26FB 33,633 on 2026-09-23). Create WO (D-FB-39) fills in with no frontend change; the "Not synced" wording becomes "Not in Fishbowl" in a follow-up once PROD passes the coverage check.
**Deploy note:** ships together with D-PRICE-49's 1.5.0 (never deployed); Matt sets `KITS_SYNC_ENABLED=true` on skyserver at deploy — the nightly kits push is wanted for the 2026-10-01 Rev 82 release. TEST's kits secrets were removed on 2026-09-23 so no TEST run can reach the live kits site.
**Files:** tools/fishbowl-bridge/src/index.mjs, src/skynet.mjs, src/queries.mjs, src/config.mjs (1.6.0), package.json, .env.example, README (+ test file if the bridge has a test convention); Docs/Decisions.md. No SkyNet migration.

**Deployment confirmed (2026-09-23):** bridge 1.6.0 on skyserver at 21:27 UTC after a TEST run from Matt's PC (1,284 rows, 218 zero, 0 stale) and a PROD dry run skipped by choice. PROD coverage: 1,306 rows (was 443), 1,178 linked to SkyNet parts (was 317), 0 stale (was 113), 223 zero rows, no error; all eight probe parts present including SK4FB13S (53) and SK203C22 current at 2,979. Missing 3 = Fishbowl products filed under other part numbers (SK244-116, SK244-216, SK2600-TS1); no-row 22 = SkyNet numbers Fishbowl does not know. KITS_SYNC_ENABLED=true set on skyserver; first nightly kits push due 02:10 on 2026-09-24. 1.4.0 kept at D:\SkyNetBridge_v1.4.0_2026-09-23.

**Notes from the build (CC, 2026-09-23):** (1) `tools/fishbowl-bridge` had no test files, but the repo convention (`node <path>.test.mjs`, plain `node:assert`, no framework) already reaches outside `src/lib` — `supabase/functions/sync-kits/plan.test.mjs` — so the pure function landed as `src/inventory.mjs` with `src/inventory.test.mjs` beside it, 37 assertions, no shim needed because the module has no imports. It caught a real defect while being written: `Number(null)` is `0`, so the part-id guard copied from the old poller would have turned a null `partId` into part 0; the module now rejects null before coercing. (2) `queries.mjs` had no string-quoting helper — every existing interpolation is numeric (`idList`) or a bridge-controlled date — so `strList` is new: it doubles quotes and escapes backslashes, and emits `NULL` for an empty list because `IN ()` is a MySQL syntax error. Part numbers come from SkyNet's own `parts` table and carry spaces and slashes (`SK2600FW-SET 2`, `10RX1/2THBS`), so they are escaped rather than trusted. (3) The inventory query keeps Fishbowl's own column casing (`q.QTYONHAND`) rather than the brief's lower-case form — MySQL identifiers are case-insensitive, and leaving the proven spellings alone keeps the diff to `FROM`/`JOIN`/`WHERE`. `partId` had to move from `q.PARTID` to `p.id`, which is null on a left-join miss. (4) `INVENTORY_DRY_RUN` logs each probe part explicitly, including the ones with no row, so a part missing from scope is visible rather than silent, and names the unknown-to-Fishbowl numbers (sorted, capped at 100) rather than only counting them. Also fixed: `openPartIds()` paged without an ORDER BY, so a page boundary could skip a part; both pagers now order on the column they page.

### D-FB-41 — Inventory chip wording after bridge 1.6.0: "Not in Fishbowl" (2026-09-23)
**What:** With D-FB-40 live, a part with no `fb_part_inventory` row is not in Fishbowl (or filed there under another part number), so the no-row state is renamed `not_in_fishbowl` and reads "Not in Fishbowl" (`FB not in FB` compact) with a tooltip saying so. The stale tooltip no longer talks about scope — nothing can leave scope now — and points at the bridge instead. Order Queue unchanged.
**Why:** D-FB-39a's "Not synced from Fishbowl" was true only while the bridge read sales-order parts alone.
**Files:** src/lib/fishbowl.js, src/components/FbInventoryChip.jsx, src/lib/fbInventory.test.mjs, Docs/Decisions.md.

## Session close — 2026-09-22/23 (D-SCHED-27 → D-FB-41)

*Appended at conversation close. Records what the round entries above do not: deployment state, open findings, queued work, and operating notes. Nothing here rewrites an entry.*

### Deployment record — D-SCHED-27 (2026-09-22/23)
Migration `2026-09-22_D-SCHED-27_length_family_history.sql` v1.1 applied to PROD by Matt: 306 parts / 28 families keyed, sources 198 rule_desc / 108 rule_tail / 0 manual; `part_machine_stats` 21 → 168 visible runs; five new views `authenticated / SELECT` only, no anon rows; SK4C4S → SK4C#S / 4, SK26CS6 → SK26CS# / 6. TEST received v1.0 plus `2026-09-22_D-SCHED-27_TEST_hotfix_source_and_grants.sql` (v1.0's trigger relabelled the seed as `manual` — 0 / 0 / 305 — and left the inherited ALL privileges on the views; both fixed in v1.1, hotfix filed TEST-only). Frontend fast-forwarded to `main` at `461394d`; D-SCHED-27a, D-FB-39/39a/39b and D-CODATE-02c followed on 2026-09-23 with `main` == `test` throughout.

### D-RLS-VIEWS01 — Finding: anon grants on owner-run PROD views (2026-09-22, OPEN)
**What was found:** on PROD, `part_machine_stats`, `family_machine_stats`, `v_wo_dates`, `v_schedulable_jobs`, `report_job_efficiency` and `material_availability` carry ALL privileges for `anon` (and `authenticated`). Supabase default privileges grant ALL on new tables/views to `anon`, `authenticated` and `service_role`; views run as owner, so RLS on the underlying tables does not apply. D-SCHED-14 records `report_job_efficiency` as revoked from anon — on PROD it is not. The D-SCHED-27 views were created with `REVOKE ALL … FROM authenticated, anon, public; GRANT SELECT … TO authenticated` and are clean.
**Remedy queued (Matt to say go):** a read-only census of every view's anon grant, then a revoke sweep, then `ALTER DEFAULT PRIVILEGES IN SCHEMA public REVOKE ALL ON TABLES FROM anon` so new views stop inheriting it. S7 posture, not new policy.

### D-PRICE-49 follow-ups and Oct 1 readiness (2026-09-23)
- **Deployed:** the D-PRICE-49 nightly kits push reached skyserver for the first time with bridge 1.6.0 (D-FB-40); `KITS_SYNC_ENABLED=true` set explicitly in skyserver's `.env`. First nightly run due 02:10 skyserver time on 2026-09-24 (Rev 81: 13 Common Sets, $13.19–$18.37) — the rehearsal for 2026-10-01, when Rev 82 (331 kits in book, 314 priced) goes to the kits site by the same run. Check: `2026-09-23_DIAG_kits_push_check_READONLY.sql` (expects a `nightly` row, `dry_run = false`, no error).
- **TEST safety (done 2026-09-23):** TEST's `sync-kits` was deployed and TEST's `pricing_kit_prices` and Rev 81 (13 priced kits) all existed, so only TEST's `KITS_SUPABASE_URL` stood between a TEST bridge run — or a publish on test-skynet — and the live kits site. Matt removed `KITS_SUPABASE_URL` and `KITS_SERVICE_ROLE_KEY` from TEST; TEST's `kits_sync_runs` shows dry runs only, so nothing was ever written. The PC bridge `.env` carries `KITS_SYNC_ENABLED=false` as a second lock.
- **Hardening queued (write-path changes, CC round when Matt calls it):** (1) flip the default — only an explicit `KITS_SYNC_ENABLED=true` enables the push (absent = off; today only the literal `false` disables); (2) `sync-kits` refuses to write unless it runs on PROD, so a copy with the wrong secret cannot reach the site; (3) write the `kits_sync_runs` row first and update it at the end — today the site writes happen before the log row, so a failure mid-run leaves writes and no record; (4) surface nightly failures somewhere visible — today the bridge swallows them (log-only, never `fb_sync_state.last_error`).
- **Before Oct 1:** 17 Rev 82 kits are unresolved by nine components and would carry a STALE badge: NAS1097AD4-5C (B35LC40P/S), LOCTITE271 (B35UC40P/S), SK2600-SWS (B60-LP1P/S, B60-LP2P/S), SK294290-2-160W (B95-C1), SSP-32 (C150C2800P, C172C2800P), MS24693-C4 (C182-C6/C6P), MS51957-28 (C310-C3/C3P), VANSRVKIT (LANCAIR-C1P-F), and 85-11-140-16 / 85-11-200-16 / 85-14-060-16 / 85-32-150-18 (PA18-C2). Section 4 of the kits check watches the count fall to 0.

### D-SCHED-28 / D-SCHED-29 — queued (defined 2026-09-22/23, not built)
- **D-SCHED-28:** (a) weekend behaviour per machine as an advisory badge with sample size — the Monday-pickup measure is real but noisy: within-job medians run −0.5 to +0.9 weekday-equivalents per weekend day on 2–14 spans per machine, the machine-level version is confounded by part mix (Nexturn 7's apparent 1.83 was slow SK244-42 weekdays against fast SK4C Mondays), and a Monday-out finishing lead is handled by classifying span hours weekday/weekend; machine monitoring supersedes the proxy; (b) material sufficiency — qty ÷ family median pieces-per-bar (SK4C#S ≈ 130, range 46–250) vs `material_availability`; (c) family campaigning — other pool jobs in the same length family ("queue it behind this one; same bar, same tooling"). Reliability badge held by Matt.
- **D-SCHED-29:** weekend-aware duration (weekday steady rate × weekdays + weekend yield × weekends) promoted into the duration math and the cascade only after `v_schedule_estimate_accuracy` shows it beats today's calendar-rate + uniform-days model.

### Bridge operations notes (2026-09-23)
- Source: `tools/fishbowl-bridge/`; skyserver runs a deployed copy at `D:\SkyNetBridge`. Deploy = zip the folder's contents (no `node_modules`, `.env`, `logs`) → extract over `D:\SkyNetBridge` → `npm install --omit=dev` → `sc stop` / `sc start SkyNetFishbowlBridge` from an elevated cmd. `nssm` is not on skyserver's PATH (it lives in `C:\Users\admin\Downloads\nssm-2.24\`); `sc` does the same job. Runbook: `2026-09-23_Bridge_1.6.0_SkyServer_Deploy_Runbook.md`. Backup of 1.4.0 at `D:\SkyNetBridge_v1.4.0_2026-09-23` (delete after a week).
- PC runs against TEST (the D-PRICE-26 route): `SB_URL` = TEST, `SESSION_MODE=per_cycle`, `KITS_SYNC_ENABLED=false`, `INVENTORY_DRY_RUN=true` for the first start. Skyserver's own `.env` runs `SESSION_MODE=per_cycle` too — leave it.
- Bridge 1.6.0 first-cycle facts (PROD): 1,306 rows; 22 SkyNet numbers Fishbowl does not know (NED R&D PRODUCT, SK201/203, SK2018-A21, SK2018-A22, SK244-116, SK244-216, SK244-INS, SK245C161, SK2600-38W, SK2600-TS1, SK29693, SK400CGP-01, SK4C17, SK4C18, SK4C18C, SK4C19, SK4C20, SK4FW2S, TEST-MANUAL-001, TEST-SS-001, TEST-ST-001, TOOLS FOR ASSEMBLY — the SK4C17–20 cups and SK4FW2S look like spelling gaps worth reconciling); 3 Fishbowl products filed under other part numbers (SK244-116 → "SK241-16 OLD", SK244-216 → "SK242-16 OLD", SK2600-TS1 → no part) — a product→part mapping in the poller would pick up the first two.

### Read-only diagnostics delivered this session (suggested home: Docs/diagnostics/)
`2026-09-17_DIAG_length_family_run_history_READONLY.sql` (family run history for any part; edit the literal); `2026-09-23_DIAG_fb_inventory_coverage_READONLY.sql` (Fishbowl inventory reaching SkyNet — the D-FB-40 pass/fail, header carries baseline and acceptance); `2026-09-23_DIAG_FISHBOWL_inventory_scope_READONLY.sql` (runs against Fishbowl's MySQL — sizing); `2026-09-23_DIAG_kits_push_check_READONLY.sql` (kits push: bridge state, last runs, what the book in effect would publish, Oct 1 preview). Each is a single SQL-Editor statement returning labelled sections with expected values in the header.

### D-KIOSK-04 — PIN-in session enforcement is scoped to the station class (2026-09-24)
**What:** Machine Kiosk completeLogin and Finishing handlePinSubmit no longer deactivate every kiosk_sessions row for a non-admin operator. A machine-kiosk PIN-in deactivates the operator's other active sessions whose machine is not machine_type 'finishing' (one machine at a time, any device — enforcement unchanged); a Finishing PIN-in deactivates only the operator's other active finishing sessions. Rows are selected with an embedded machines(machine_type), filtered client-side and deactivated by id (.in), not via .not('…','in',…), which silently matches nothing in Supabase JS. A select error falls back to the old deactivate-all. The Kiosk 30-second session poll gains .order(logged_in_at desc).limit(1) to match the realtime recheck, so duplicate active rows cannot force a wrongful logout.
**Why:** Harry Swinnes (machinist, also covering finishing) runs the Nexturn 5 kiosk and Finishing on one PC. Each PIN-in at one station deactivated his session at the other, whose listener force-logged it out ('session_displaced'); kiosk_sessions on 2026-09-24 shows five alternating logins in four minutes. Each machine-kiosk logout calls signOut({scope:'local'}), which — because every SkyNet page in a browser shares one supabase-js session — also signed him out of the main app (four logout?scope=local calls in the edge logs, none from the main app's global Sign Out). Finishing's forced logout cleared its on-screen queue, batches, notes and verified counts; no database rows were lost (J-000244 and J-000254 completed normally).
**Deferred (D-KIOSK-05 candidate):** auth-storage isolation. The machine Kiosk adopts a kiosk-minted JWT with setSession, calls stopAutoRefresh, and signs out with scope 'local' — all against the same storage key the main app uses, on the stated assumption "kiosk pages are dedicated". Any PC that uses a machine kiosk and the main app in the same browser will still lose the main-app session at every kiosk PIN-in and logout. Proper fix is a kiosk-only Supabase client with its own storageKey (and the lib helpers the kiosk calls accepting a client); interim workaround is a separate browser or browser profile for the machine kiosk.
**Files:** src/pages/Kiosk.jsx, src/pages/Finishing.jsx, Docs/Decisions.md.

### D-KIOSK-ZERO01 — Kiosk number inputs start empty, never with a typeable 0 (2026-09-24)
**What:** Machine Kiosk materialForm.bars_loaded initialises and resets to '' instead of 0, so the Bars Loaded field shows its grey placeholder rather than a literal 0; existing validation already rejects ''. Thirteen other Kiosk number inputs whose state is numeric (secondary-op good/bad pieces, bars remaining at completion, downtime edit hours/mins, extend duration hours/mins, edit-downtime hours/mins, tool-change hours/mins, log-downtime hours/mins) now bind value to `x || ''` with placeholder "0" — display only: state and onChange are unchanged, so every save path receives the same number as before and an empty field still saves 0. Finishing.jsx inputs already bind to '' and were not changed.
