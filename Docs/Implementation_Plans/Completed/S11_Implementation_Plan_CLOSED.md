# SkyNet MES — Sprint 11 Implementation Plan — CLOSED 2026-09-05
...
**Status:** CLOSED. Built Sept 3–5, 2026; PROD live 2026-09-05 (19 days ahead of target). See §14 Close-out.
...
## 14. Close-out (2026-09-05)

### What shipped vs the plan
| Batch | Planned (TEST) | Actual | Notes |
|---|---|---|---|
| A — schema, loader, seed, bridge v1.3 | Sept 8 | Sept 4 | Loader in Python (one-shot; seed SQL is the artifact). `kit_components` name collision with the Kit Registry → `price_kit_components`. Line-type bug (30 vs 12) found and fixed in B.1. |
| B — shell, Lookup, Catalog, Customers | Sept 12 | Sept 4 | Verified against the RPC (mirror check T-04 passed). |
| B.1 — Quote Builder, stats, exclusions | — (added) | Sept 4 | From Matt's first walkthrough: Lookup → Quote Builder with recommendation + any-column override; Top-10; "Who buys this part"; non-sellables excluded; sets fixed (cross-book components); resizable sidebar. |
| C1 / C1.1 / C1.2 — price lists, pictures | Sept 18 (price sheet) | Sept 4 | Price sheet became a persistent, numbered **price list** whose changed prices become customer specials (D-PRICE-29); 198 pictures extracted from the workbook; gallery + lightbox; admin add/remove. |
| C2 / C2.1 — saved quotes, PDF branding | Batch E (October) | Sept 4 | Pulled forward per Matt (quotes ahead of price sheets). Skybolt logo + brand colours; WinAnsi-safe text; save-before-PDF. |
| C3 — Price Books editor | Sept 18 | Sept 5 | Clone / uplift / edit / diff / schedule / Fishbowl Products CSV; ladders remain SQL. |
| D — PROD cutover | Sept 22–24 | **Sept 5** | Runbook executed; fingerprints identical to TEST; bridge v1.3.0 on skyserver; 70,227 history lines (all type 10). D.1: Catalog first tab. |

### Tests
T-01 … T-16 passed on TEST (T-04 mirror check; T-15 CSV import wizard column names still to be confirmed by Matt on the first real import). Every SQL file was executed end-to-end on a scratch Postgres 16 before delivery; every PDF builder was rendered in Node before delivery.

### Decisions
D-PRICE-01 … 37 in Docs/Decisions.md. Corrections recorded append-only: D-PRICE-26 → 28 (line types), D-PRICE-21 → 29, D-PRICE-23 → 32/33.

### Deviations from the plan
- Quoting (Phase E) delivered inside the sprint; price-sheet export delivered as price lists with persistence and specials.
- Loader is Python, not Node (Matt never needs to run it).
- Tier seeds are candidates ("confirm" note) — confirmation is a follow-on task for April / Sawyer, not a sprint deliverable.
- Fishbowl customer master cleanup (13,979 inactivated, salesman set, hcollins retired) happened inside the sprint; the 459-name rename is parked with a DB runbook because the Customers import cannot rename.
- Cutover 19 days early; Sept 24 remained the outside date.

### Open at close (carried to Pricing_Portal_Backlog_2026-09-05.md)
Phase F write-back and retirement of Fishbowl's 150 qty-triggered tier rules; SES email for quotes/price lists; tier confirmation workflow (108 seeds, 259 mixed incl. Irwin); Won quote → Fishbowl SO; ladder editor and Price Books housekeeping; hourly history option; Fishbowl-side hygiene (120 unpriced resale items, 115 list-price mismatches, 6,189 never-sold products, Block 9 check, customer rename + SkyNet name sweep).

### Oct 1, 2026
Rev 82 (+15% on every catalog Each) is scheduled; activation is by date. Morning check: `/pricing` header reads "In effect today: Rev 82 — Oct 2026"; SK2600-1 walk-in → $6.07; Price Books ▸ Rev 82 ▸ Fishbowl Products CSV → Fishbowl import until Phase F; reissue price lists for customers who had one.
